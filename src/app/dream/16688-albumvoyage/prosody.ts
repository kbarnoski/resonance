...
//
// Adapted from 16672-scriptorium. The offset mapping keeps the same reading: a
// word's `offsetFrac` is a fraction *within its line's current region of a
// recording* (0..1), not an absolute cut into a whole buffer. In albumvoyage the
// engine resolves that fraction against the line's region of ITS assigned album
// track, then adds two migrations on top — a slow always-on READ-DRIFT (minutes)
// and a golden-ratio per-loop grain step — so the same word never cuts the same
// grain twice, and the choir slowly walks deeper through the album over minutes.
//
