// source.ts — Karel's real solo-piano recording loader, an offline "felt piano"
// synth fallback, and CataRT-style grain-corpus analysis. Client-side only.
// This is a READ of an existing public GET route (/api/audio/<id>); nothing is
// recorded or sent, so there is no API route of our own and no guard needed.
//
// Loader + analysis are adapted verbatim from the lab's proven piano loaders
// (see src/app/dream/718-duet-paths/audio.ts).
...
/** Decode a user-dropped audio File into an AudioBuffer (null on failure). */
export async function decodeUserFile(
...
/**
 * Render a ~12s gentle solo-piano-like buffer (multi-partial body + hammer
 * transients + sub drone) with an OfflineAudioContext so the grain corpus
 * always has real harmonic + percussive content to scan when the network is
 * unavailable. Mirrors the lab's proven fallback render.
 */
...
export function buildGrainCorpus(buffer: AudioBuffer): Grain[] {
  ...
  const grainSecs = 0.22; // ~220ms windows
  const strideSecs = 0.11; // 50% overlap
  ...
  if (grains.length > 700) {
    const step = grains.length / 700;
    const trimmed: Grain[] = [];
    for (let i = 0; i < 700; i++) trimmed.push(grains[Math.floor(i * step)]);
    return trimmed;
  }
  return grains;
}
...
/**
 * Concatenative "selection": retrieve the corpus grain best matching a target
 * MIDI pitch AND target brightness, preferring singing (louder/tonal) grains.
 * This is the CataRT navigation step — the flock is the agent that drives the
 * target descriptors each frame.
 */
export function selectGrain(
  corpus: Grain[],
  targetMidi: number,
  ...
  for (const g of corpus) {
    ...
    const brightCost = Math.abs(g.brightness - targetBrightness) * 3;
    const cost = pitchCost + brightCost - g.rms * 1.5;
    if (cost < bestCost) {
      bestCost = cost;
      best = g;
    }
