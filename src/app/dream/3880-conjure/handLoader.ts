// Minimal typed loader for MediaPipe Tasks-Vision HandLandmarker, loaded from
// a CDN at runtime (never bundled by webpack — note the webpackIgnore comment
// below). We declare only the shapes we use; the CDN module is treated as
// `unknown` then narrowed. Mirrors the pattern already used by
// src/app/dream/862-kids-solfege-signs/handLoader.ts elsewhere in this lab.
...
export async function makeHandLandmarker(): Promise<HandLandmarkerLike> {
  // CDN ESM module (webpackIgnore keeps it a runtime import, never bundled).
