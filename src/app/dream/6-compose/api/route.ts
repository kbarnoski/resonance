...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const tags = body.tags?.trim() || "ambient, meditative, piano, 70 BPM";
    const duration = body.duration ?? 30;

    // ACE-Step text-to-music: tags = style/genre description, lyrics "[inst]" = instrumental.
    // Base endpoint (no /audio-to-audio suffix) for text-driven generation.
