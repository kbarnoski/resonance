...
export default function FaceOrganPage() {
  ...
  return (
    <main className="relative h-dvh w-screen overflow-hidden bg-[#fbeee0] text-foreground">
      <canvas ref={canvasRef} className="absolute inset-0 h-full w-full" aria-hidden />

      {/* ── Idle / start panel ─────────────────────────────────────────────── */}
      {phase !== "running" && (
        <div className="absolute inset-0 flex items-center justify-center px-6">
          <div className="max-w-xl rounded-3xl bg-black/45 p-8 backdrop-blur-md ring-1 ring-border">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <button
                ...
                className="inline-flex min-h-[44px] min-w-[44px] items-center justify-center rounded-xl bg-card px-4 py-2.5 text-base font-medium text-black transition hover:bg-accent disabled:opacity-60"
              >
              ...
              <button
                ...
                onClick={() => begin(false)}
                disabled={phase === "starting"}
                className="inline-flex min-h-[44px] min-w-[44px] items-center justify-center rounded-xl bg-muted px-4 py-2.5 text-base font-medium text-foreground ring-1 ring-border transition hover:bg-accent disabled:opacity-60"
              >
      ...
      {phase === "running" && (
        <>
          <div className="pointer-events-none absolute left-6 top-6 select-none">
            <h1 className="text-2xl font-semibold text-foreground drop-shadow-[0_1px_6px_rgba(120,60,20,0.55)]">
              Face Organ
            </h1>
            {mode === "face" ? (
              <p className="mt-1 text-base text-violet-300/95 drop-shadow-[0_1px_4px_rgba(80,40,10,0.6)]">
          ...
          <div className="pointer-events-none absolute right-6 top-6 select-none text-right">
            <div className="text-6xl font-bold text-foreground drop-shadow-[0_2px_10px_rgba(120,60,20,0.6)]">
              {hud.vowel}
            </div>
            <div className="mt-1 text-2xl font-semibold text-foreground drop-shadow-[0_1px_6px_rgba(120,60,20,0.55)]">
              {hud.note}
            </div>
            <div className="mt-2 ml-auto h-2 w-40 overflow-hidden rounded-full bg-black/25 ring-1 ring-border">
              <div
                className="h-full rounded-full bg-gradient-to-r from-violet-300 to-violet-300 transition-[width] duration-100"
          ...
          {mode === "pads" && (
            <div className="absolute inset-x-0 bottom-8 flex flex-col items-center gap-4 px-6">
                  ...
                  <button
                    ...
                    className={`inline-flex min-h-[64px] min-w-[64px] items-center justify-center rounded-2xl px-4 py-2.5 text-2xl font-bold ring-1 backdrop-blur-md transition ${
                      activePad === p.name
                        ...
                        : "bg-black/35 text-foreground ring-border hover:bg-black/45"
                    }`}
                  >
                    {p.name}
                  </button>
              ...
              <div className="flex w-full max-w-md items-center gap-3 rounded-2xl bg-black/35 px-4 py-2.5 ring-1 ring-border backdrop-blur-md">
                <span className="text-base font-medium text-foreground">pitch</span>
                <input
                  type="range"
                  min={0}
                  max={1}
                ...
                <button
                  ...
                  className="inline-flex min-h-[44px] min-w-[44px] items-center justify-center rounded-xl bg-muted px-4 py-2.5 text-base font-medium text-foreground ring-1 ring-border transition hover:bg-accent"
                >
                  Shimmer
                </button>
              </div>
      ...
      <button
        ...
        className="absolute bottom-6 right-6 inline-flex min-h-[44px] min-w-[44px] items-center justify-center rounded-xl bg-black/45 px-4 py-2.5 text-base text-foreground ring-1 ring-border backdrop-blur-md transition hover:text-foreground"
      >
      ...
      {showNotes && (
        <div className="absolute inset-0 z-10 flex items-start justify-center overflow-y-auto bg-black/80 px-6 py-16 backdrop-blur-md">
          <div className="max-w-2xl text-foreground">
            <h2 className="text-2xl font-semibold text-foreground">Face Organ — design notes</h2>
            <p className="mt-4 text-base leading-relaxed">
              <span className="text-foreground">The question.</span> What if your face
              were a hands-free vocal instrument — could you &ldquo;sing&rdquo; a
              shimmering choir with your expressions alone?
            </p>
            <p className="mt-4 text-base leading-relaxed">
              <span className="text-foreground">Technique.</span> A real formant synth:
            ...
            <p className="mt-4 text-base leading-relaxed">
              <span className="text-foreground">Mapping.</span> jawOpen → gate +
              openness · smile ↔ pucker → vowel front/back · brow raise → pitch ·
              head roll → pan &amp; bend · blink → accent.
            </p>
            <p className="mt-4 text-base leading-relaxed">
              <span className="text-foreground">Inspiration.</span> The facial-
