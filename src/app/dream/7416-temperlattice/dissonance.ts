/**
 * 7416 · Temperlattice — Sethares dissonance-curve scale derivation (cycle 3).
 *
 * The scale is NOT assumed. It is DERIVED from the current timbre: given the
 * live spectrum (partial ratios + amplitudes) we sweep a second copy of that
 * same spectrum across an octave and, at every interval, sum the pairwise
 * sensory dissonance (Plomp–Levelt / Sethares). The LOCAL MINIMA of that curve
 * are the consonant interval steps for THIS timbre — its natural scale. Reshape
 * the spectrum and the whole curve re-flows, so the scale re-lays-out, and the
 * crystal lattice's sites physically migrate to the new valleys.
 *
 * The core math (dissmeasure / computeSpectrum / the constants) is the verified
 * cycle-2 formula carried forward unchanged from 6808-spectrascale.
 *
 ...
 * Bandwidth" (1965); Guillet, "Elementary spectrum for the dissonance curve,"
 * J. Mathematics and Music (2026).
 */
...
/** Resolution of the swept dissonance curve (one sample per interval step). */
...
/** A playable scale degree: the tonic plus every derived valley, ascending. */
export interface Degree {
  ratio: number;
  cents: number;
  /** 0..1 valley depth (deeper valley = brighter site). */
  bright: number;
  nearest: string;
}
 ...
 * over all pairs. Standard Sethares `dissmeasure` with the ~0.24
 * critical-bandwidth point and the two-exponential roughness model.
 */
...
export function computeSpectrum(p: TimbreParams): Spectrum {
  ...
  const norm = Math.sqrt(1 + p.inharm);
  const ratios: number[] = [];
  const amps: number[] = [];
...
/** Nearest low-complexity just ratio (q ≤ 16, complexity-penalised) to cents. */
 ...
 * record total sensory dissonance at each interval. The local minima ARE the
 * derived scale steps for this timbre.
 */
...
/**
 * Local minima with a prominence gate (~1.2% of span) so the octave-approach
 * plateau and numerical wiggles don't register as spurious steps; minima closer
 * than ~18 cents are merged, keeping the deeper one.
 */
...
/**
 * The full playable scale for a spectrum: the tonic (unison) first, then every
 * derived valley in ascending order. Site brightness is the valley depth,
 * normalised so the deepest consonance valley glows brightest.
 */
export function scaleDegrees(scale: Scale): Degree[] {
  const span = Math.max(1e-9, scale.maxD - scale.minD);
  const degrees: Degree[] = [
    { ratio: 1, cents: 0, bright: 1, nearest: "1/1" },
  ];
  for (const v of scale.valleys) {
    degrees.push({
      ratio: v.ratio,
      cents: v.cents,
      bright: Math.max(0.15, Math.min(1, (scale.maxD - v.value) / span)),
      nearest: v.nearest,
    });
  }
  return degrees;
