// ─────────────────────────────────────────────────────────────────────────────
// marchingCubes.ts — a from-scratch marching-cubes polygonizer.
//
// This is NOT three/examples/MarchingCubes and NOT a raymarch shader — it is the
// classic Lorensen–Cline (1987) algorithm written out by hand: the full 256-entry
// EDGE_TABLE and 256-row TRI_TABLE below are the standard published lookup tables
// (they are just data — reproduced here so the module is fully self-contained).
//
// polygonize() marches one 2π³ chunk of the gyroid field on an N³ cell grid,
// linearly interpolates each edge crossing at the isolevel `t`, and emits
// positions + analytic-gradient normals as flat Float32Arrays ready for a
// THREE.BufferGeometry. Because the field is 2π-periodic, the chunk tiles
// seamlessly across an instanced lattice.
// ─────────────────────────────────────────────────────────────────────────────

import { field, gradient } from "./gyroid";

// Which of the 12 edges are intersected, indexed by the 8-bit cube corner mask.
// prettier-ignore
...
// For each cube corner mask, the list of edge triples forming the triangles
// (−1 terminated). 256 rows, up to 5 triangles (15 edges) each.
// prettier-ignore
...
// The 8 cube corners, in the ordering the tables above assume.
// prettier-ignore
...
// Each edge connects two corners (indices into CORNER).
// prettier-ignore
...
export interface MarchResult {
  ...
  /** Number of vertices (positions.length / 3). Always a multiple of 3. */
  vertexCount: number;
  /** Triangle count and grid resolution, for diagnostics. */
  triangleCount: number;
  resolution: number;
}

export interface MarchOptions {
  /** Cells per axis (grid is res³ cells, (res+1)³ samples). */
  resolution: number;
  /** Isolevel t. Gyroid surface is t = 0. */
  isolevel?: number;
  /** Extent of the chunk on each axis. Must be 2π to tile seamlessly. */
  period?: number;
  /** gyroid → Schwarz-P blend, 0..1. */
  morph?: number;
}

/**
 * March one chunk of the field and return interleaved-free position/normal
 * Float32Arrays. Normals are the normalized analytic gradient at each emitted
 * vertex (recomputed at the interpolated crossing, not corner-averaged).
 */
export function polygonize(opts: MarchOptions): MarchResult {
  const res = Math.max(4, Math.floor(opts.resolution));
  const iso = opts.isolevel ?? 0;
  const period = opts.period ?? Math.PI * 2;
  const morph = opts.morph ?? 0;

  const n = res + 1; // samples per axis
  const h = period / res; // world size of one cell

  // Precompute the scalar field on the sample lattice.
  ...
  for (let k = 0; k < n; k++) {
    ...
    for (let j = 0; j < n; j++) {
      const y = j * h;
      for (let i = 0; i < n; i++) {
        values[idx(i, j, k)] = field(i * h, y, z, morph);
      }
    }
  }

  // Growable scratch arrays (over-allocated, trimmed at the end).
  const positions: number[] = [];
  const normals: number[] = [];

  // Reused per-cell scratch: corner values, positions, and edge-vertex cache.
  const cval = new Float64Array(8);
  const cpos: [number, number, number][] = Array.from({ length: 8 }, () => [
    0, 0, 0,
  ]);
  const evert: ([number, number, number] | null)[] = new Array(12).fill(null);
  const grad: [number, number, number] = [0, 0, 0];

  for (let k = 0; k < res; k++) {
    for (let j = 0; j < res; j++) {
      for (let i = 0; i < res; i++) {
        // Gather the 8 corner values / positions and build the corner mask.
        let cubeindex = 0;
        for (let c = 0; c < 8; c++) {
        ...
        if (edges === 0) continue; // cell fully inside or outside — no surface

        // Interpolate a vertex on every intersected edge.
        for (let e = 0; e < 12; e++) {
          if ((edges & (1 << e)) === 0) {
            evert[e] = null;
            continue;
          ...
          const vb = cval[b];
          const denom = vb - va;
          const mu = Math.abs(denom) < 1e-12 ? 0.5 : (iso - va) / denom;
          const pa = cpos[a];
          const pb = cpos[b];
          evert[e] = [
            pa[0] + mu * (pb[0] - pa[0]),
        ...
        for (let t = 0; tris[t] !== -1; t += 3) {
          for (let w = 0; w < 3; w++) {
            const p = evert[tris[t + w]];
            if (!p) continue; // defensive; table guarantees presence
            positions.push(p[0], p[1], p[2]);
            gradient(p[0], p[1], p[2], grad, morph);
            let gx = grad[0];
            let gy = grad[1];
            let gz = grad[2];
