...
//
// Ported from cycles 1 & 2 (the proven approach), in the spirit of Simon Dixon's
// MATCH (2005): rather than building the full N×M cost matrix offline, we grow the
// alignment path one note at a time as live notes arrive, within a bounded window.
//
// On each incoming played pitch we:
//   1) compute a local cost matching that pitch against reference cells inside a
//      bounded window ahead of the last committed column,
//   2) extend the accumulated-cost frontier with the classic recurrence
//      D(i,j) = cost(i,j) + min( D(i-1,j), D(i,j-1), D(i-1,j-1) ),
//   3) commit the reference column with the lowest accumulated cost (forward-only),
//   4) derive local tempo from the SLOPE of the warping path,
//   5) report a CONFIDENCE derived from the recent normalized local cost — this is
//      what the supervisor reads to decide whether DTW is trustworthy. When the
//      soloist plays a wrong-note run, the local cost spikes and confidence falls,
//      which is precisely the signal that DTW has lost the plot.
//
// Like cycle 2, it also tracks expressive parameters (velocity, articulation) so
// the audio can couple to dynamics + articulation.

import type { ScoreNote } from "./score"

// ─── Cost between a played pitch and a reference note ────────────────────────
// Pitch-class aware: exact match is 0, octave errors are cheap, semitone errors
// are expensive (so an off-key wrong note registers as a clear mismatch).
...
export interface FollowResult {
  ...
  localCost: number    // raw pitch cost at committed cell (0 = perfect match)
  confidence: number   // 0..1 smoothed confidence (1 = matching cleanly)
...
export class OnlineDTW {
  ...
  // Smoothed confidence (EMA over instantaneous match quality).
  private _confidence = 1

  // Smoothed expressive parameters (EMA).
  private _smoothedVelocity = 64
  private _smoothedArticulation = 0.7
  ...
  // Feed one live played note. Returns committed alignment + tempo + expression
  // + confidence.
  step(
    playedMidi: number,
    velocity: number,
    ...
    // ── Confidence ────────────────────────────────────────────────────────────
    // Instantaneous quality: 1 for a perfect match, decaying as local cost rises.
    // A semitone error (cost 1) → ~0.37; a tritone (cost 6) → near 0.
    const instant = Math.exp(-localCost * 0.9)
    // EMA so a single slip dips but a run collapses confidence — this is what
    // makes the supervisor hand control to the HMM during a wrong-note RUN.
    this._confidence = this._confidence * 0.55 + instant * 0.45
  ...
  get committed(): number { return Math.max(0, this.committedCol) }
