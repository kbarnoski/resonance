// ─────────────────────────────────────────────────────────────────────────────
// silhouette.ts — model-free body sensing for orbit•room.
//
...
//
// With no camera (denied / unavailable) a deterministic seeded virtual performer
// drifts the centroid along a slow Lissajous orbit, so the room map animates and
// the sources orbit on their own — never blank, never silent once sound begins.
// ─────────────────────────────────────────────────────────────────────────────
...
export interface SilhouetteStart {
  rig: SilhouetteRig;
  /** Human-readable reason we fell back to the virtual performer, or null. */
...
export function makeVirtualRig(seed: number): SilhouetteRig {
  ...
  return {
    ...
    read(dt: number): CentroidReading {
      t += dt;
      const x = 0.5 + 0.34 * Math.sin(t * fx * Math.PI * 2 + px);
      const y = 0.5 + 0.16 * Math.sin(t * fy * Math.PI * 2 + py);
      const area = 0.34 + 0.22 * (0.5 + 0.5 * Math.sin(t * fa * Math.PI * 2 + pa));
      return { x, y, area };
    },
    stop() {
      /* nothing to release */
...
function makeCameraRig(
  ...
  const read = (dt: number): CentroidReading => {
    ...
    // Mirror horizontally so stepping to YOUR left moves the dot left.
    c2d.save();
    c2d.scale(-1, 1);
...
/**
 * Try the front camera; on any failure fall back to the deterministic performer.
...
export async function startSilhouette(seed: number): Promise<SilhouetteStart> {
  ...
  try {
    ...
    return {
      rig: makeVirtualRig(seed),
      fallbackReason:
        "Camera not available — running the virtual performer instead.",
    };
  }
}
