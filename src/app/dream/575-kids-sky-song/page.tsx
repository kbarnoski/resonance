...
export default function KidsSkySongPage() {
  ...
  return (
    <div className="relative h-dvh w-screen overflow-hidden bg-black font-mono text-white select-none">
      ...
      <div className="pointer-events-none absolute left-4 top-4 z-10 max-w-[70%]">
        <h1 className="font-serif text-2xl text-white/95 drop-shadow sm:text-3xl">
        ...
        {weatherError && (
          <p className="mt-1 text-base text-rose-300">couldn&rsquo;t reach the weather</p>
        )}
        {weatherError && (
          <p className="text-base text-white/55">showing an example sky</p>
        )}
      </div>

      {/* ── WebGL fallback notice ── */}
      {!usingWebGL && (
        <p className="pointer-events-none absolute right-4 top-4 z-10 text-base text-rose-300">
      ...
      {!started && (
        <div className="pointer-events-none absolute inset-0 z-10 flex flex-col items-center justify-center">
          <span className="relative flex h-28 w-28 items-center justify-center">
            ...
            <span className="absolute inline-flex h-20 w-20 rounded-full bg-white/15" />
            {/* play glyph */}
            <svg
              viewBox="0 0 24 24"
              className="relative h-12 w-12 text-white/90 drop-shadow"
              fill="currentColor"
              aria-hidden="true"
            >
          ...
          <p className="mt-4 text-base text-white/75">touch the sky</p>
        </div>
      )}
      ...
      <Link
        href="/dream"
        className="absolute bottom-4 left-4 z-10 inline-flex min-h-[44px] items-center rounded-full bg-white/10 px-4 py-2.5 text-base text-white/75 backdrop-blur transition hover:bg-white/20 hover:text-white"
      >
        &larr; back
      </Link>

      {/* ── credit line (outside play area, bottom-right) ── */}
      <p className="pointer-events-none absolute bottom-4 right-4 z-10 text-base text-white/55">
