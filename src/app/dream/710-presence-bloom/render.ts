...
// ── Presence Bloom · particle renderer ───────────────────────────────────────
...
export interface Attractor {
  ...
  pulse: number; // 0..1.4 bloom burst
}
...
fn vs(@builtin(vertex_index) vi: u32, @builtin(instance_index) ii: u32) -> VOut {
  ...
  o.uv = c;
  // Cool→warm gradient by speed; brighter with energy. Luminous teal→gold.
  let warm = clamp(speed * 0.012, 0.0, 1.0);
  let base = mix(vec3<f32>(0.18, 0.55, 0.95), vec3<f32>(1.0, 0.78, 0.32), warm);
...
// accreting-attractor behaviour, additive blending, trailing fade.
const FB_COUNT = 3600;
  ...
  frame(attractors: Attractor[], energy: number, t: number, dt: number) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (g > 0.02) {
        ...
        const warmth = Math.min(1, al[i]);
        grd.addColorStop(
          0,
          `rgba(${120 + warmth * 130}, ${180 + warmth * 40}, ${
            220 - warmth * 80
          }, ${Math.min(0.5, g * 0.55)})`,
        );
    ...
    for (let i = 0; i < FB_COUNT; i++) {
      ...
      const r = Math.floor((60 + warm * 195) * bri);
      const g = Math.floor((150 + warm * 50) * bri);
      const b = Math.floor((235 - warm * 130) * bri);
