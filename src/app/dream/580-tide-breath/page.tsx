    ...
    <div className="relative w-full min-h-screen bg-black overflow-hidden">
      ...
      {(phase === "idle" || phase === "loading") && (
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6 z-20">
          ...
          <h1 className="font-serif text-3xl md:text-4xl text-white/95 mb-3 tracking-wide z-10">
            Tide Breath
          </h1>
          <p className="text-base text-white/75 max-w-sm mb-1 leading-relaxed z-10">
          ...
          <p className="text-sm text-white/60 max-w-xs mb-10 leading-relaxed z-10">
            Location optional &middot; no key &middot; no server
          </p>

          {loadError && (
            <p className="mb-5 text-sm text-rose-300 max-w-xs z-10">{loadError}</p>
          )}

          <button
            onClick={() => void handlePlay()}
            disabled={phase === "loading"}
            className="z-10 min-h-[52px] px-8 py-3 rounded-full text-lg font-medium text-white
          ...
          <p className="mt-4 text-sm text-white/55 z-10">
          ...
          {showNotes && (
            <div className="z-10 mt-4 max-w-md text-left text-sm text-white/75 leading-relaxed
                            bg-white/5 border border-white/10 rounded-xl px-5 py-4 backdrop-blur-sm">
              <p className="text-white/90 font-medium mb-2">How it works</p>
              <p className="mb-2">
              ...
              <p className="mb-2">
                <span className="text-white/90">Sound:</span> Six oscillator pairs tuned to a
              ...
              <p className="mb-2">
                <span className="text-white/90">Visual:</span> A sum-of-sines water surface
          ...
          <Link href="/dream" className="absolute bottom-6 text-xs text-white/40 hover:text-white/70 z-10">
      ...
      {phase === "playing" && (
        <>
          ...
          <div className="absolute top-5 right-5 z-20 flex flex-col items-end gap-2 select-none">
            <button
              ...
              className="min-h-[44px] px-4 py-2.5 text-xs uppercase tracking-wider
                         text-white/70 border border-white/20 hover:border-white/50
                         hover:text-white rounded transition"
            >
              Stop
            </button>
            <Link href="/dream"
          ...
          <div className="absolute bottom-0 left-0 right-0 z-20 px-5 pb-6 pt-10"
            ...
            {statusText && (
              <p className={`text-sm font-mono mb-1 ${isLive ? "text-emerald-300/95" : "text-amber-300/95"}`}>
                {isLive ? "●" : "○"} {statusText}
              </p>
            )}
            {marine && (
              <div className="flex flex-wrap gap-x-5 gap-y-0.5 text-xs font-mono text-white/60">
                {swellPeriod !== null && (
          ...
          {showNotes && (
            <div className="absolute top-16 right-5 z-20 max-w-xs text-sm text-white/75
                            leading-relaxed bg-black/70 border border-white/10 rounded-xl
                            px-5 py-4 backdrop-blur-sm">
              <p className="text-white/90 font-medium mb-2 text-base">Design notes</p>
              <p className="mb-2">
