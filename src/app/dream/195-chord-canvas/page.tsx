...
// Pitch-class display and hue (chromatic wheel: each PC = 30°)
...
const TEMPLATES = [
  { quality: "",     vecs: buildAllVecs([0, 4, 7])      },
  { quality: "m",    vecs: buildAllVecs([0, 3, 7])      },
  { quality: "7",    vecs: buildAllVecs([0, 4, 7, 10])  },
  { quality: "m7",   vecs: buildAllVecs([0, 3, 7, 10])  },
  { quality: "maj7", vecs: buildAllVecs([0, 4, 7, 11])  },
  { quality: "dim",  vecs: buildAllVecs([0, 3, 6])      },
];

// ── Demo sequence: Dm7 → G7 → Cmaj7 → Bdim, 3 reps ─────────────────────────
const DEMO_NOTES = [
  { midis: [50, 53, 57, 60], dur: 2.2 },  // Dm7
  { midis: [43, 47, 50, 53], dur: 2.2 },  // G7
  { midis: [48, 52, 55, 59], dur: 3.0 },  // Cmaj7
  { midis: [47, 50, 53],     dur: 2.2 },  // Bdim
];
...
// ── Types ─────────────────────────────────────────────────────────────────────
interface Block { name: string; root: number; quality: string; duration: number }
...
export default function ChordCanvasPage() {
  ...
  const loop = useCallback(() => {
    ...
    const name = PC_NAMES[root] + getQS(quality).label;

    if (name !== curNameRef.current && now - curStartRef.current > HOLD_MIN) {
      if (curNameRef.current !== "") {
        blocksRef.current.push({
          name:     curNameRef.current,
    ...
    // Active chord grows from right edge
    if (curNameRef.current) {
    ...
    for (let i = blocksRef.current.length - 1; i >= 0; i--) {
      ...
      if (bW > 26) {
        ...
        ctx2d.textAlign = "center";
        ctx2d.textBaseline = "middle";
        ctx2d.fillText(bl.name, lx + bW / 2, TL_H / 2);
        ctx2d.textBaseline = "alphabetic";
      }
      rx = lx;
    }
    ...
    for (let pc = 0; pc < 12; pc++) {
      ...
      ctx2d.font = "9px monospace";
      ctx2d.textAlign = "center";
      ctx2d.fillText(PC_NAMES[pc], 12 + pc * barW + (barW - 2) / 2, H - 5);
    }
  ...
  // ── Display color for hero chord name ─────────────────────────────────────
  const qs           = getQS(cQual);
  const displayHue   = (PC_HUE[cRoot] + qs.hs + 360) % 360;
  const displayColor = `hsl(${displayHue},${Math.min(qs.sat + 22, 100)}%,${Math.min(qs.lit + 30, 84)}%)`;
  ...
  return (
    <div className="flex flex-col h-screen bg-[#04040a] text-white overflow-hidden">
      ...
      <div className="flex justify-between items-center px-5 py-2 text-white/55 text-xs font-mono shrink-0 border-t border-white/5">
        <span>195-chord-canvas · cycle 227 · zero deps · zero api</span>
        <span>chroma · 6 chord types · color timeline</span>
      </div>
    </div>
  );
}
