 ...
 *    rise      0.18–0.35  depth + fall speed + warp rising
...
export function evalArc(timeSeconds: number): ArcState {
  ...
  const rise = smoothstep(0.18, 0.4, progress);
  const settle = 1 - smoothstep(0.62, 0.92, progress);
  const peak = rise * settle;
  ...
  // warp breathes in on the rise, eases at the landing
