...
//
// Adapted from 16656-tonguescript, but the offset mapping is reinterpreted for
// the FORM-VOYAGE reading: a word's `offsetFrac` is now a fraction *within its
// line's current region of the recording* (0..1), not an absolute cut into the
// whole buffer. The engine resolves that fraction against region [slot/L,
// (slot+1)/L] * bufferDuration and drifts it a golden step every loop cycle, so
// the FIRST line reads the opening of Karel's piece, later lines read deeper, and
// no word ever cuts exactly the same grain twice.
//
// Nothing here touches the Web Audio API — engine.ts binds these abstract plans
// to the one decoded buffer. Keeping it pure makes the mapping legible + testable.
// ─────────────────────────────────────────────────────────────────────────────
 ...
 * same region of Karel's piece with the same prosody.
 */
...
/**
 * Deterministic stereo pan for a line: spread the choir by its slot across the
 * field, then nudge left/right by its vowel density so vowel-bright lines lean
 * one way. Stable given (slot, L, vowelDensity). Range roughly [-0.9, 0.9].
 */
