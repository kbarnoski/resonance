...
**Deepen your own visionary breakthrough.** One continuous dial — `depth ∈ [0,1]` —
...
`depth` drives **both** the geometry and the sound at once:

| depth | word | look | sound |
|------|------|------|-------|
...
2. On load an **auto-breath** slowly oscillates the depth up and down (a seeded
...
4. The slim meter on the right shows the current depth plus a one-word state label
...
scrolled toward the camera forever (planes recycle behind the camera). As `depth`
rises:

- **forward speed** climbs (`~1.6 → 26` units/s, eased on depth²),
...
recomputed each frame from the depth (≤1440 instances — cheap).
...
level open with depth; an endless-rising `shepard` glissando whose ascent rate and
brightness rise with depth; both washed through the code-built `convolutionVoid`
reverb (wet rises with depth). Master is capped at 0.18 through a
...
- **`prefers-reduced-motion`** slows everything: speed cap, twist, bloom and the
  depth-smoothing time constant are all reduced.
- **No WebGL** → an on-brand `text-destructive` notice, and the audio keeps
  playing (the depth loop and meter stay live so you can still deepen the sound).
...
- Cells at `depth ≈ 1` can sweep very close to the camera as they rush past; this is
...
- Depth handoff from user back to auto-breath is smoothed by the depth lerp, so it
