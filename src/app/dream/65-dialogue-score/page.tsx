...
type Contour = "ascending" | "descending" | "arch" | "valley" | "neutral";
...
function detectContour(notes: NoteEvent[]): Contour {
  if (notes.length < 3) return "neutral";

  const midis = notes.map(n => Math.round(n.midi));

  let totalDelta = 0;
  for (let i = 0; i < midis.length - 1; i++) {
    totalDelta += midis[i + 1] - midis[i];
  }
  const avgDelta = totalDelta / (midis.length - 1);

  if (avgDelta > 0.9) return "ascending";
  if (avgDelta < -0.9) return "descending";

  // Check for arch vs valley via first-half / second-half delta comparison
  const half = Math.floor(midis.length / 2);
  let firstDelta = 0;
  for (let i = 0; i < half - 1; i++) firstDelta += midis[i + 1] - midis[i];
  let secondDelta = 0;
  for (let i = half; i < midis.length - 1; i++) secondDelta += midis[i + 1] - midis[i];

  if (firstDelta > 1.5 && secondDelta < -1.5) return "arch";
  if (firstDelta < -1.5 && secondDelta > 1.5) return "valley";
...
const PENTA_UP = [2, 3, 5, 7];
const PENTA_DOWN = [-7, -5, -3, -2];
...
function resolveStep(
  ...
  if (transitions && transitions.size > 0 && Math.random() > 0.25) {
    // Filter Markov candidates by direction
    const filtered: Array<[number, number]> = [];
    for (const [midi, count] of transitions) {
      const delta = midi - current;
      if ((dir === "up" && delta > 0) || (dir === "down" && delta < 0) || dir === "any") {
        filtered.push([midi, count]);
      }
    }
    if (filtered.length > 0) {
      const total = filtered.reduce((a, [, c]) => a + c, 0);
      let r = Math.random() * total;
      for (const [midi, count] of filtered) {
        r -= count;
        if (r <= 0) return midi;
      }
      return filtered[0][0];
  ...
  // Pentatonic fallback with direction bias
  const steps = dir === "up" ? PENTA_UP : dir === "down" ? PENTA_DOWN : PENTA_STEPS;
  return current + steps[Math.floor(Math.random() * steps.length)];
}

function contourDir(contour: Contour, i: number, total: number): "up" | "down" | "any" {
  if (contour === "ascending") return "up";
  if (contour === "descending") return "down";
  if (contour === "arch") return i < total / 2 ? "up" : "down";
  if (contour === "valley") return i < total / 2 ? "down" : "up";
  return "any";
}

function generateContourResponse(
  lastNotes: NoteEvent[],
  table: Map<number, Map<number, number>>,
  length: number,
  contour: Contour
  ...
  for (let i = 0; i < length; i++) {
    const dir = contourDir(contour, i, length);
    const next = resolveStep(current, table, dir);
    result.push(Math.max(36, Math.min(84, next)));
...
// ── demo phrase — clearly ascending C major scale ─────────────────────────────

const DEMO_PHRASE: Array<{ midi: number; durMs: number }> = [
  { midi: 60, durMs: 280 }, // C4
  { midi: 62, durMs: 250 }, // D4
  { midi: 64, durMs: 250 }, // E4
  { midi: 65, durMs: 250 }, // F4
  { midi: 67, durMs: 280 }, // G4
  { midi: 69, durMs: 280 }, // A4
  { midi: 71, durMs: 280 }, // B4
  { midi: 72, durMs: 560 }, // C5 (held)
];
...
export default function DialogueScore() {
  ...
  const [userContour, setUserContour] = useState<Contour>("neutral");
  const [ariaContour, setAriaContour] = useState<Contour>("neutral");
  ...
  const triggerResponse = useCallback(() => {
    ...
    const contour = detectContour(userNotes);
    const responseLen = Math.max(8, Math.min(16, userNotes.length));
    const responseMidis = generateContourResponse(userNotes, newTable, responseLen, contour);

    setUserContour(contour);
    setAriaContour(contour);
    setShowContour(true);
    ...
    setStatusText(`${CONTOUR_LABEL[contour]} — Aria mirrors the shape`);
  ...
  const startDemo = useCallback(() => {
    ...
    setPhase("listening");
    phaseRef.current = "listening";
    setStatusText("— demo: C major scale ascending...");
    ...
    setTimeout(() => {
      if (!triggeredRef.current && phaseRef.current === "listening") {
        ...
        setStatusText("Detecting contour...");
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      let trimIdx = 0;
      while (trimIdx < bars.length && bars[trimIdx].endMs < cutoff && !bars[trimIdx].ghost) trimIdx++;
    ...
    <div style={{ background: "#050508", minHeight: "100vh", color: "#fff", fontFamily: "monospace", overflow: "hidden", position: "relative" }}>
      ...
      <div style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 10, padding: "14px 20px", background: "linear-gradient(to bottom, rgba(5,5,8,0.92) 0%, transparent 100%)" }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
          <span style={{ fontSize: 17, fontWeight: "bold", letterSpacing: 2 }}>DIALOGUE SCORE</span>
          <span style={{ fontSize: 11, color: "rgba(255,255,255,0.38)" }}>contour · dialogue · cycle 81</span>
        </div>
        ...
        {/* Contour indicator */}
        {showContour && (
          <div style={{ marginTop: 6, display: "flex", gap: 18, fontSize: 11 }}>
            <span>
              <span style={{ color: "rgba(255,170,60,0.65)" }}>your phrase </span>
              <span style={{ color: "#ffaa3c", fontWeight: "bold" }}>{CONTOUR_LABEL[userContour]}</span>
            </span>
            <span style={{ color: "rgba(255,255,255,0.22)" }}>→ aria mirrors →</span>
            <span>
              <span style={{ color: "rgba(80,160,255,0.65)" }}>aria responds </span>
              <span style={{ color: "#50a0ff", fontWeight: "bold" }}>{CONTOUR_LABEL[ariaContour]}</span>
            </span>
          </div>
        )}
      </div>
      ...
      {phase === "idle" && (
        <div style={{ position: "fixed", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 20, gap: 16 }}>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.50)", textAlign: "center", maxWidth: 440, lineHeight: 1.85, margin: "0 0 8px" }}>
            Play a melody. Aria detects its&nbsp;
            <span style={{ color: "rgba(255,255,255,0.8)" }}>shape</span>:<br />
            ascending ↗ · descending ↘ · arch ∧ · valley ∨<br />
            Her response follows the same contour.<br />
            Ghost notes appear before she plays — watch the plan unfold.
          </p>
          <p style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", textAlign: "center", maxWidth: 380, lineHeight: 1.7, margin: "0 0 4px" }}>
            Demo plays C major scale ascending — Aria responds ascending.<br />
            Inspired by &ldquo;Dialogue in Resonance&rdquo; (arxiv 2505.16259, 2026).
          </p>
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 10, padding: "10px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", background: "linear-gradient(to top, rgba(5,5,8,0.88) 0%, transparent 100%)" }}>
        <span style={{ fontSize: 10, color: "rgba(255,255,255,0.20)" }}>
          top = you (warm) · bottom = aria (dashed = ghost, solid = played) · center = now
        </span>
