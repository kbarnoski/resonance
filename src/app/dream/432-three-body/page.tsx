...
export default function ThreeBody() {
  ...
  return (
    <main className="relative h-[100dvh] w-full overflow-hidden bg-[#05060d] text-white">
      <canvas
        ref={canvasRef}
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 z-10 flex flex-col gap-1 p-5">
        <h1 className="font-mono text-2xl font-semibold text-white/95 sm:text-3xl">
          432 · Three-Body
        </h1>
      ...
      {phase === "running" && (
        <>
          {audioBlocked && (
            <p className="absolute left-1/2 top-24 z-20 -translate-x-1/2 px-4 text-center text-base text-rose-300">
      ...
      {/* Design notes */}
      <details className="absolute right-4 top-5 z-20 max-w-xs rounded-lg bg-black/50 p-1 font-mono text-base text-white/75 backdrop-blur-sm">
        <summary className="cursor-pointer px-2 py-1.5 text-white/75">design notes</summary>
        <div className="space-y-2 p-2 text-white/75">
          <p>
      ...
      <Link
        href="/dream"
        className="absolute bottom-4 right-4 z-20 font-mono text-base text-white/55 transition hover:text-white/75"
      >
        ← dream lab
      </Link>
    </main>
