// audio.ts — real-audio loader + realtime analysis, in three tiers so the
// crucible is never blank or silent:
//   1. a Path recording id  → fetch the EXISTING read-only GET /api/audio/<id>
//   2. a dropped / picked local file → decode it
//   3. synth fallback → an offline-rendered gentle piano arpeggio
//
// Playback runs source → dry + convolver reverb → compressor/limiter →
// analyser → destination (the limiter protects ears). Per frame it exposes RMS
// energy (→ target temperature), positive spectral flux (onset → kinetic
// shock) and spectral centroid (brightness → hue bias).
//
// READ-ONLY of an existing public GET route. No mic, no new API, no recording.
// Self-contained — the pattern is copied from 1148-light-accretion, not imported.
...
/** Local seeded PRNG (kept self-contained — no cross-file import). */
 ...
 * audio bytes. Returns null on ANY failure so the caller can fall back.
 */
...
export async function renderFallbackBuffer(sampleRate = 44100): Promise<AudioBuffer> {
  ...
  const rand = makeRand(0x0f0cc12e);
...
/** Build a short synthesized impulse response for a soft reverb. */
function buildImpulse(ctx: AudioContext, seconds = 2.6): AudioBuffer {
  const rate = ctx.sampleRate;
  const len = Math.floor(seconds * rate);
  const impulse = ctx.createBuffer(2, len, rate);
  const rand = makeRand(0x51e5b0ba);
 ...
 *   source → [dry + convolver wet] → compressor(limiter) → analyser → dest
...
export class AudioEngine {
  ...
  /**
   * Per-frame analysis → RMS energy, positive spectral flux (onset strength)
   * and spectral centroid (brightness), each normalized to ~0..1.
   */
  getFrame(): AudioFrame {
