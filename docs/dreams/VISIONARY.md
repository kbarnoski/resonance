# VISIONARY.md — Altered-States Direction & Research
...
Immersive AV experiences that evoke the **phenomenology** of altered states — the visuals, the sound, the felt sense — using only WebGL/WebGPU + Web Audio in the browser. **No substances — the screen and speakers do the work.**

Two poles, both wanted — pick across the whole spectrum, don't camp on one:

- **INTENSE** — breakthrough-state geometry, peak visionary fractal melt, ego-dissolution. High entropy, high saturation, overwhelming.
- **COSMIC / AMBIENT** — meditative boundlessness, the near-death tunnel-toward-light, dissolution into a calm void, hypnagogic drift. Slow, weightless, luminous.

Every prototype should be able to answer: *which altered state is this, and which pole?* Tag it in STATE.md (e.g. `state: breakthrough-state · pole: intense` or `state: meditative-boundless · pole: cosmic-ambient`). **Tags name the STATE, never a substance.**

This direction **supersedes** the "spread across journeys / use real Paths music / pull back on voice gen" emphasis from 2026-05-21 as the *theme* driver — but all the **hard rules still hold** (Ambition & Diversity Mandate, scope fence, build-before-commit, immutability). **Audio source is now MANDATORY, not optional (AGENT.md ABSOLUTE rule 10, set by Karel 2026-08-14):** every prototype's sound MUST come from Karel's VERIFIED catalog — the **16 verified tracks** in `../_shared/welcomeHome` (`REAL_TRACKS`: Welcome Home ×13 + Snowflake ×3; the 17th St / Folsom St sessions are quarantined-unverified and "Sketches" is Joseph's music — neither is ever a valid source) via `loadRealTrackBuffer` — no synth/oscillator audio — until he interjects with new direction. His music is the carrier wave for the state; use `../_shared/trackAnalysis` (`loadTrackAnalysis`) to let the *harmony, melody, and form* of the piece drive the visionary geometry. The Ambition & Diversity Mandate is fully compatible: these states demand never-used techniques (4D raymarching, log-polar warps, reaction-diffusion, convolution-reverb voids) and multi-subsystem builds.

---

## The single most load-bearing finding

**All visionary geometry is one stripe/hexagon pattern seen through a log-polar warp.** (Bressloff–Cowan, building on Klüver's four "form constants.") The retina→V1 cortical map is a complex-logarithm transform: concentric circles ↔ vertical cortical stripes, radial spokes ↔ horizontal stripes, spirals ↔ diagonals, lattices ↔ hexagons.

**So:** generate plane-wave stripes or a hexagonal Turing pattern in cortical space, apply an **inverse log-polar warp (`exp()`)** to screen space, and you get **tunnels, funnels, spirals, and honeycomb lattices from one shader.** This is the foundational visionary-geometry engine. Build it once into `_shared/`, drive it everywhere.

Klüver's four form constants: **(1) lattices/gratings/honeycombs, (2) cobwebs, (3) tunnels/funnels/cones, (4) spirals.** They recur across the visionary/breakthrough state, the slow visionary ascent, warm-ecstatic states, migraine, hypnagogia, *and flicker* — because they're a property of visual cortex, not any external cause.

---

## Cluster 1 — Visionary / serotonergic-type states (breakthrough geometry, fractal melt, warm affect)

**Phenomenology.**
- **The visionary / breakthrough state** — the extreme. Threshold → **chrysanthemum** (dense unfolding fractal "flower") → "waiting room" shimmering architecture → **breakthrough** into a fully-realized hyperspace with "more axes than physical reality allows." Hyperbolic / negatively-curved **saddle surfaces** everywhere ("hyperdimensional bedsheets blowing in the wind"), all wallpaper symmetry groups, ultra-saturated **neon/iridescent jeweled** color, "more real than real," frequent visionary-entity contact.
- **The slow visionary ascent** — slower, drifting. Surfaces **breathing** and drifting, **fractal recursion**, moiré, **color trails/tracers** (positive afterimages lagging motion), persistent **visual snow**. Gradual arc: long onset, long plateau, many hours total.
- **The warm mystic pole** — organic, emotionally-toned. Speckles bloom into morphing kaleidoscopic fractals; open-eye it *enhances* natural fractal structure (foliage, bark, clouds); patterns shift **with the user's affect**, arriving in warm ecstatic waves.

**Neuroscience → parameters.**
- **Entropic brain / REBUS** (Carhart-Harris, Friston): the visionary state raises cortical entropy and relaxes high-level priors. → Over a session, *increase noise amplitude/octaves, flow speed, and rate of pattern reorganization*; loosen symmetry at peak.
...
## Cluster 2 — Dissociative & near-death states (the void pole)
...
**Phenomenology.**
- **The dissociative void** — depersonalization, derealization, dissolution of body schema, floating/OOB, ego dissolution, severe **time dilation**, "melting into the surrounding," cosmic oneness. Quieter and more internal than the visionary pole. Deep dissociation is the far end: a spacious, weightless emptiness with no body-anchor at all.
...
- **Tunnel-toward-light** candidate mechanism: retinal/cortical hypoxia constricting the field to a bright center (the literal "tunnel vision" of ischemia). **Gamma surge at death** (Borjigin, PNAS 2013 rat / 2023 human) — a ~30s surge of high-frequency gamma resembling conscious/dreaming states. *(Endogenous-tryptamine theories of NDE remain speculative — do not present as fact.)*

**Neuroscience → parameters.**
- The dissociative void = **NMDA-antagonist-type disconnection** → thalamocortical disconnection / sensory-gating breakdown. → **Decouple the audio envelope from visual motion** (desync the two streams the brain normally binds); **thin sensory density** (sparse geometry); **slow time** (stretch every curve).
...
Especially relevant because the brain produces these **with no external trigger at all** — the closest existing proof that screens/sound can move consciousness.
...
**The flicker payload (KEY).** Stroboscopic light on closed eyes reliably evokes the **Klüver form constants purely through light** — most intense around **~10 Hz**. Lineage: Purkinje → Grey Walter → Brion Gysin's **Dreamachine** (1959) → Collective Act's **Dreamachine** (2022). The **Ganzfeld** (uniform unstructured field → brain amplifies neural noise into imagery) and **Ganzflicker** (uniform field pulsed at alpha 8–12 Hz) are directly screen-implementable.
...
- **Music is the carrier wave, not background.** Kaelen's *The hidden therapist* (Imperial, 2018): in visionary-state therapy the *music* guides the journey; outcome tracked liking + resonance + openness, **not** intensity of the state itself. → The soundtrack is the structural spine; slave visuals to it via FFT; place the emotional peak of the music at the visual breakthrough.
...
Mirror the clinical visionary-session playlist phases. The same skeleton serves both poles — intense pushes entropy/saturation hard at peak; cosmic-ambient stays slow, weightless, luminous.

1. **Onset (0–15%)** — stillness, low entropy, faint breathing, subtle grain. Drone/ambient.
2. **Rise (15–35%)** — drift and trails begin; form constants emerge faintly; warmth + saturation rise.
...
Don't let "visionary" collapse into one look. Rotate deliberately across:
- **State**: breakthrough-state · slow visionary ascent · warm-ecstatic · dissociative void · NDE · jhāna/meditation · hypnagogia · lucid-dream.
...
- **Palette**: neon-iridescent (breakthrough) · pastel-drift (slow ascent) · warm-organic (warm-ecstatic) · dark-void-luminous (dissociative/NDE) · soft-white-boundless (meditation) · dim-warped (hypnagogia).
...
Visionary / serotonergic-type states: Form constant (Wikipedia, `en.wikipedia.org/wiki/Form_constant`) · Bressloff–Cowan "Uncoiling the spiral" (plus.maths.org, search title) · Quanta: the math of visionary geometry (quantamagazine.org, 2018-07-30) · Breakthrough-state phenomenology (Nature Sci Reports, `s41598-022-11999-8`) · QRI hyperbolic geometry of the breakthrough state (qualiacomputing.com, 2016-12-12) · REBUS (PMC `PMC6588209`) · Entropic brain (Frontiers Hum. Neurosci. 2014.00020) · Visionary-state neuroimaging (PNAS, `10.1073/pnas.1518377113`) · Kaelen "hidden therapist" (Beckley Foundation, 2018-02-08)

Dissociative/NDE: [Moody, Life After Life (Wikipedia)](https://en.wikipedia.org/wiki/Life_After_Life_(Moody_book)) · [AWARE study (Psi Encyclopedia)](https://psi-encyclopedia.spr.ac.uk/articles/aware-nde-study/) · [Gamma surge at death, PNAS 2013](https://www.pnas.org/doi/10.1073/pnas.1308285110) · [Human gamma surge, PNAS 2023](https://www.pnas.org/doi/10.1073/pnas.2216268120) · [Dissociative ego dissolution (Frontiers)](https://www.frontiersin.org/journals/human-neuroscience/articles/10.3389/fnhum.2017.00245/full)
