...
## ⭐ Newest (Cycle 382 — **kids** build · DEEP, 3 coupled-pond explorers. The jury's #2 ask answered for kids: *deepen an existing lab-first by extension, don't grab a new primitive.* Wave Field spine **CYCLE 2** — `478`'s one FDTD pond becomes TWO coupled wave fields that talk through a sympathetic channel, with a floating lily-pad that physically carries sound across. Renderer moved off the jury-banned three.js → Canvas2D. Two siblings — a child-draggable coupling vine (+ WebGPU compute) + a clean cooperative two-kids core — banked to IDEAS §382.)
