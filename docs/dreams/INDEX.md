...
## ⭐ Newest (Cycle 303 — adult build · DEEP, 2 approaches)

- **[/dream/302-mirror-canon-round](/dream/302-mirror-canon-round)** — Mirror Canon (Round). `demoable`
  **Conduct a four-voice round sung entirely by past versions of yourself.** Perform a body-phrase in front of the camera, tap **"Add a voice to the round,"** and a *past-you* enters a beat later singing the **same** material in canon — until you've stacked and conducted a four-voice round of your own selves in a matte wooden mirror. This is **cycle 1 of the lab's "Mirror Canon" thread**, a deliberate multi-cycle **deepening of the standout `287-mirror-choir`** (the 2026-06-03 JURY's #1 provocation: *"pick ONE thing — 287 is the obvious candidate, README promises record-a-canon-with-past-you — and deepen it over 2–3 cycles instead of shipping orphans"*). The 287 engine is **re-implemented in-folder** (`pose.ts` + `audio.ts`, no cross-import) and a genuinely new subsystem is added on top: a **stacked-round canon-memory engine**. Four subsystems → one pipeline: (1) **camera → body**, MediaPipe Pose Landmarker Lite loaded at runtime from the jsDelivr CDN via a `webpackIgnore` dynamic import (**adds no npm dep**, never touches package.json) → 33 smoothed landmarks; hand height → pitch on a strict **D-Dorian** chord stack, arm-span openness → vowel, register from body height; (2) a **Klatt/Peterson-Barney formant choir** — each voice a `sawtooth` glottal pulse (+ a fifth) through three parallel `bandpass` formant filters (F1/F2/F3, Q 9–12), an always-on warm sine pad so it's never silent, a tape-style Frippertronics delay, the whole master through a `DynamicsCompressor` limiter so it can **never** get harsh, all params gliding via `setTargetAtTime`; (3) the **stacked-round memory engine** — a locked **bar clock** (4 beats @ 72 BPM ≈ 3.3 s/bar) records the live **parameter stream** (pitch/vowel/register/energy/pose, 96 snapshots/bar) for one bar, commits it as a canon voice, and **enters it offset by a fixed beat** (`sampleLoop` at `barPhase − offset`) on one shared grid — a true round, **no drift**, up to 4 voices, with live **conduct** controls (per-voice mute/solo, glide-faded, + Clear round); (4) a **matte wooden-mirror** renderer — a tessellated grid of warm amber tiles on near-black, **pure Canvas2D source-over (NO additive, NO glow, NO bloom, NO three.js — the Rozin aesthetic)**; the live silhouette is the brightest amber, each canon voice a dimmer, distinctly-**tinted** past-you fanned across the mirror. It **accumulates** — genuinely fuller at minute 2 than at second 5; that evolving state is the point (the jury's 275-memory-loom depth ask). Degrades at every step: **no camera / MediaPipe blocked / no pose within ~3 s → a ghost performer** loops a hand-authored phrase through the *identical* pipeline and **auto-commits two voices** so the whole round demos itself on a phone with zero sensors (reason in readable `text-rose-300`); `getImageData`/`OffscreenCanvas` failures fall back to the landmark-hull ellipse; AudioContext + camera both requested **inside the Begin tap** (iOS-safe). Winner of a **DEEP** 2-approach fire; the parallel sibling **`301-mirror-canon-phase`** (same engine, **Steve Reich *Piano Phase*** drift — loops at slightly different rates so the past-yous phase apart and the piece never repeats) is build-reviewed and banked in IDEAS.md as the **designated cycle-2 deepening** (fold it in as a Round⇄Phase mode toggle). **Ambition 3/5**: ≥3-subsystems(#2) + named-reference(#3) + multi-cycle-commitment(#4). **Diversity**: INPUT=camera/body (not touch) · OUTPUT=matte wooden-mirror (deliberate **thread-continuity** with 287's Rozin signature, not a fresh similar pick) · TECHNIQUE=formant-synthesis + **stacked-round canon memory** (the genuinely new axis) · VIBE=adult/embodied/meditative-ritual — dodges the jury's banned touch/three.js/pentatonic-poke. References: **Daniel Rozin, *Wooden Mirror* (1999)**; the **round/canon** tradition (*Frère Jacques*, *Sumer Is Icumen In*); **Frippertronics** / Robert Fripp; **Pauline Oliveros, *Deep Listening***; the **loop-pedal one-person-choir** (Jacob Collier; Ariana Grande's BOSS RC-505); **LUMIA** (arXiv:2512.17228, Dec 2025). Born from RESEARCH §303. Adult · zero new deps · zero API route · no guard · 6.89 kB. **Build-verified, not browser-verified** — unverified: MediaPipe CDN load reliability on real hardware, live-camera tracking latency, loudness balance / formant intelligibility, and whether the staggered round reads as "a choir of my selves" vs. a wash.
