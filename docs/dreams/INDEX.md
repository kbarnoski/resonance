...
## ⭐ Newest (Cycle 393 — **adult** build · WIDE, 3 divergent explorers. Ships **`520-singing-dune`**: *one living dune of sand under its own gravity — you tip the world and it avalanches and re-settles forever, never the same, and the dune's own motion IS the sound (the real "song of dunes").* The lab's **first actual continuum simulation** — a **WebGPU compute MLS-MPM** granular solid (APIC + Drucker-Prager plasticity so it piles/avalanches, not splashes); **device-orientation tilt** drives gravity; a **booming-dune granular drone** (shear→pitch, kinetic energy→density, avalanche→grain bursts) with no harmonic progression — the drama is all in the sand. **tilt INPUT + WebGPU-compute OUTPUT** · ONE presence · NEVER resolves. Answers JURY 2026-06-11 #2 (no resolution) + #3 (extend WebGPU, don't migrate renderers) + #5 (one presence). CPU MLS-MPM fallback (2k grains, Canvas2D) settles + sounds **on load** with no WebGPU (covers iOS) and no interaction, so the 06:30 glance always shows a breathing dune. Finishes the MLS-MPM upgrade that loved **`84-wave-fluid`** deferred. 2 siblings banked IDEAS §393: **`521-overpass`** (live ISS pass → a lone three.js voice crossing the sky — real-world-data, the cleanest resurrect) + `522-chladni-voice` (Chladni point-cloud — but overlaps existing `19`/`165-cymatics`, a repeat the WIDE fan-out caught).)
