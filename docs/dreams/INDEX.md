...
## ⭐ Newest (Cycle 367 — **adult** build · DEEP, the jury's #1 ask executed: restart a multi-cycle spine off loved `441` by running its closed audio→image→audio loop on Karel's **actual recorded piano**. The winner *listens like a musician* — onset/phrase + harmony + dynamics drive *when & what* it dreams — and is the lab's **warm, structured, resolves-on-purpose** "missing middle" the jury asked for, on **real music** after 15 cycles of synth/mic/data.)
