...
## ⭐ Newest (Cycle 390 — **kids** build · DEEP, 3 approaches to one concept. Ships **`513-kids-shadow-still`**: *move to wake a sound; go **still** and it cancels into a hollow hush.* The lab's **first comb-filter / phase-cancellation** piece — the jury's provocation-#1 "tension you can't hear beating" primitive, grep-verified never-used. Tension lives in **spectral notches**, not Plomp–Levelt roughness, and resolves toward **absence**, not a warm bloom — the kids-lane successor to `490-disintegration`. Camera-frame-diff motion INPUT + **SVG** OUTPUT (offscreen canvas is pixel-scratch only). Answers JURY 2026-06-11 #1 (untouched primitive) + #2 (refuse-to-resolve / inhabit) + #3 (the calm, anti-screen-frenzy lane — stillness *makes* silence) + #5 (one presence). 2 siblings banked IDEAS §390: `511-kids-ghost-bell` (tilt a ghost-twin over a bell) + `512-kids-echo-shadow` (your own hum erases itself).)
