...
### `327-physarum-choir` (adult) — place tones as "food" and a living slime-mold network autonomously routes between them; the chord you hear IS which sources are currently connected — the slime composes the connections `[queued — research seed from RESEARCH §318; a genuinely never-used technique (#1)]`
