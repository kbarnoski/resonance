...
**The thread:** run the closed audio→image→audio loop (born in loved `441-latent-listening-room`) on Karel's **actual recorded piano**, deepening over multiple cycles. **Cycle 1 (367, SHIPPED):** `448-piano-phrase-painter` — musical-structure listening (onset/phrase/chroma/dynamics) drives *when & what* is dreamed; image color bends the real playback back. **Cycle 2 (planned, ~369):** replace the single template-prompt with a **small multi-agent music→caption refinement** — a scene agent + a color agent + a style agent cooperate on the music-derived prompt (arXiv 2512.23320, RESEARCH §367), which also mirrors the lab's own planner→builders→critic orchestration. **Cycle 3 (planned):** close the *other* half of the loop — let the dreamed image **re-compose** the piano, not just filter it (fold in `449`'s granular image→audio loop-back below, and/or a learned image→music map à la *Art2Mus* 2602.17599). **Open dependency:** a real **Welcome Home** track ID (the runtime `journey_paths` IDs aren't in the static repo — `448` defaults to the Ghost recording `549fc519…`); the same ask blocks `424-welcome-erosion`. Karel: paste a WH recording ID and the spine runs on the album.
