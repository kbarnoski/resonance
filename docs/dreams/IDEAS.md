...
### kids-lenia-pond — a 4-year-old breeds glowing lifeforms that sing `[queued · build-verified · the bigger swing]`
...
**Status**: built + build-verified in the cycle-280 all-present build (`○ Static`, 4.7 kB; one harmless `Unused eslint-disable` warning, non-fatal). **The most surprising of the three** — "make life that sings" is the directive's "massively bigger concept." Lost to Slime Garden only because its magic hinges on Lenia *seed stability* I couldn't browser-verify pre-ship, and its creatures wander on their own agenda (less direct 4-yo cause-effect than slime crawling to your finger).
