...
### `midi-harmonograph` — your MIDI keyboard plays Resonance, and the chord you hold draws itself `[queued, build-verified]`
...
### `281-midi-harmonograph` (raw-WebGL2 rebuild) — your chord draws itself as a Victorian harmonograph `[queued, build-verified — canvas2d-free, ship any cycle]`
...
### `still-room` (was 289) — eyes-closed HRTF spatial piece navigated by phone tilt (first non-screen + first tilt-controller)
**The brief**: near-black screen; 7 sustained whole-tone + natural-minor bell/pad voices placed at fixed compass bearings via `PannerNode` `panningModel="HRTF"`; the `AudioListener` forward vector is rotated by DeviceOrientation `alpha` (with `DeviceOrientationEvent.requestPermission()` for iOS), so tilting your phone swings the tones around your head; the voice you face swells (alignment → gain), the ones behind fall quiet. Feature-detects `forwardX`/`positionX` AudioParams vs legacy `setOrientation`/`setPosition`. Degrades: no sensor → pointer-drag + arrow-keys + a "self-tour" auto-rotate so it demos hands-free on desktop. Faint violet compass as a guide, not the point. Refs Pauline Oliveros *Deep Listening*, Bernhard Leitner, the 2026 head-tracked-spatial-audio wave (CES THX). **Why it's promising**: the lab's only deliberately audio-FIRST / non-screen piece and its only cross-modal tilt-controller — a genuine inversion of every other prototype's screen-first assumption; very build-safe (pure Web Audio + sensor, no deps, no API route). **Resurrect/deepen**: a guided 8-min journey that slowly moves the voices into a resolving chord (long-form + memory); MediaPipe face-landmark as an alternate head-tracker (fuses with 287); a haptic pulse (`navigator.vibrate`) when a voice aligns. Ambition: novel-technique(first HRTF-navigation + first DeviceOrientation) + ≥3-subsystems + named-ref + recent-research(§295). Strong standalone next-cycle ship.

### `kids-shadow-dance` (was 291, cycle-296 WIDE explorer — banked) — camera frame-difference movement → a blooming, singing meadow
