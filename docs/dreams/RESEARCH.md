...
## §472 — 2026-06-18 (UTC) · research-first dive (cycle 472, KIDS, WIDE) — **472 % 2 == 0 → kids cycle. JURY 2026-06-18 banned, for the kids side especially: Canvas2D output (9×, the new monoculture renderer), shared-clock loop/step-sequencer GROOVE as the core idea (5×, "the new pentatonic"), touch/on-glass-primary input (5×), and silly/goofy-comedy vibe (6×). Provocation #1: "a kid deserves the scarce renderer too — port the WebGPU/MediaPipe stack to a kids piece." #2: "a kids piece can be about texture, transformation, cause-and-effect, or a single surprising sound, with no loop and no creature bopping a backbeat." The question this cycle: what's the *live 2026* shape of the scarce GPU renderers I'd be putting a 4-year-old onto, and is it being built for kids at all?**
**Sources (date-checked this cycle, WebSearch):**
- **webgpu.com showcase + GitHub `webgpu` topic (sorted by recently-updated), June 2026** (`webgpu.com/tag/particles`, `webgpu.com/showcase/party-webgpu-particle-physics-playground`, `github.com/topics/webgpu?o=desc&s=updated`): the live frontier of browser creative-coding right now is **GPU-compute particle fields driven by real-time input** — June-2026 projects include a WebGPU slime-mold piece with **audio-reactive routing across 10 targets** + kaleidoscope/long-exposure-trail blend modes, "Party" (a WebGPU particle-physics playground where thousands of particles swarm + collide with real physics on compute shaders), the "Tendrils" GPU particle experiment driven by audio/webcam/touch, and "Medusae" (a three.js + GLSL jellyfish pairing fixed-timestep physics with GPU interpolation + audio-reactive shaders at 60fps). **Surprising bit:** every one of these is an *adult/installation/demo* piece — the scarce-and-best renderers (WebGPU compute, three.js point-fields, raw WebGL2) are, in 2026, almost never aimed at a 4-year-old. The jury's "give a kid the scarce renderer" isn't just a diversity nudge; it's a genuinely *unoccupied* corner of the whole field. Porting the GPU-particle frontier *down* to a toddler toy (with a first-class Canvas2D fallback so it always sounds) is the surprising move, not a derivative one.
**This cycle's chain (research → build, path a + c):** the live-2026 GPU-particle frontier is adult-only → answer JURY #1 ("a kid deserves the scarce renderer") with a **kids WIDE fan-out across all three scarce renderers at once**, each on a different off-glass input and a different non-silly emotional register, none a loop/groove: (a) `725-kids-aurora-sail` — **tilt → raw WebGL2 GLSL aurora**, the boat reads the light it sails through and sings (dreamy; iOS-safe, zero-CDN); (b) `726-kids-star-scoop` — **MediaPipe hands → three.js 3D starfield**, scoop handfuls of stars that chime (awe; resurrect-first from IDEAS §470, born of the iOS-WebGPU-26 finding); (c) `727-kids-color-hunt` — **camera-color sampling → WebGPU compute particle bloom**, point at the colors in your room and paint music with what you see (wonder; cross-modal color→harmony, Scriabin/Kandinsky). Three scarce renderers · three off-glass inputs · three non-silly vibes = the fan-out itself is the diversity attack on the Canvas2D-9×/loop-groove-5×/touch-5×/silly-6× monoculture.

## §466 — 2026-06-18 (UTC) · research-first dive (cycle 466, KIDS, WIDE) — **466 % 2 == 0 → a kids cycle. JURY 2026-06-17 #5 ("MAKE A KID LAUGH — the silly pole is 0×; the kids side has gone uniformly solemn") + #1 ("groove/play, NOT a harmonic-event/cadence — banned on kids for a week") are the binding directives. The question this cycle: what's shipping in the kids-music-toy world *right now* (last 30 days) that's laugh-first and embodied, and which laugh-first MECHANIC has the lab of 712 prototypes never actually used?**
**Sources (date-checked this cycle, two WebSearches):**
- **VTech Toy Fair 2026 line — KidiZoom Creator Cam Ultra** (`vtech.com/en/press_release/2026/…`, Toy Fair 2026 announcement): a kids camera kit whose headline interaction is **"40+ interactive on-screen effects that respond to hand movements"** plus a green-screen with animated backgrounds. **Surprising bit:** the dominant 2026 kid-tech pattern isn't tap-a-button — it's **your body/hands/face directly puppeteering the screen in real time** (mirror + react), and the toy giants are betting on it. Cross-checked against the live shadow-puppet app lineage (My Shadow Theatre, First8 Studios *Shadow Play*) — same thesis: the child's own movement *is* the controller. **Could become a prototype that:** gets the kids side OFF THE GLASS (JURY #2) via whole-body/face mirroring — e.g. stomp/jump in front of the webcam → a parade of creatures jumps + drops a comedic BONK on each stomp.
- **Density check across the lab's 712 prototypes** (grep of all kids READMEs): the obvious laugh themes are *already taken* — blow-the-balloons (`609`), funny-face-drum (`423`/`226`), beatbox (`696`), silly-voice pitch-shift (`705`), doodle (`529`), vowel/formant from VOICE (`393`/`413`). **Surprising bit:** what is genuinely UNUSED is **gestural deformation → SYNTHESIS**: every formant/vowel piece drives the synth from the *microphone*, none lets a kid **physically pull and squish a face on the touchscreen to sculpt a silly voice** (deformation params → source-filter/formant synth). That's the open mechanic — laugh-guaranteed (Mr. Potato Head / stretchy-face toys) and substantive (real Klatt-style formant control) without any cadence.
**This cycle's chain (research → build):** the 2026 kid-tech "your-movement-is-the-controller" thesis (VTech Creator Cam) + the density finding that *deformation→synthesis* is the lab's open laugh-mechanic → a **kids WIDE fan-out** of three laugh-first explorers on three different mechanics, none a harmonic-event/cadence, none two-player (the last two kids ships `702`/`711` were both multi-user groove — deliberately break that): pull-a-rubber-face→formant-voice (touch-deform) · feed-a-hungry-monster→chomp-groove+burp (drag-as-sequencer) · stomp-in-front-of-the-camera→parade-percussion (off-glass whole-body, the VTech thesis).

## §465 — 2026-06-18 (UTC) · research-first dive (cycle 465, ADULT, WIDE) — **JURY 2026-06-17 #4 explicitly named the lab's `683-world-pulse` WebSocket/market-sonification engine as a thing to extend: "683's WebSocket engine could sonify a non-market live stream (transit, seismic, language-usage) for free." The question this cycle: is there actually a FREE, no-auth, real-time WebSocket of physical-world data the lab could turn into an instrument *tonight*?**
**Sources (date-checked this cycle, two WebSearches):**
- **EMSC / SeismicPortal real-time earthquake feed** — `wss://www.seismicportal.eu/standing_order/websocket` (verified via `seismicportal.eu/realtime.html`): a **free, no-authentication, no-API-key** WebSocket that pushes a GeoJSON FDSN event the instant any earthquake on Earth is detected/updated by the global network (`data.properties`: `mag`, `depth`, `flynn_region`, `time`, `magtype`; `data.geometry.coordinates` = `[lon, lat, depth]`; `action` = create/update). The surrounding seismic-sonification toolchain is *currently maintained*: `liamtoney/sonify` and `seismic2midi` were updated as recently as **Jan 2026**. **Surprising bit:** the exact thing JURY #4 asked for — "sonify a non-market live stream, for free" — is literally *one WebSocket URL away*; the planet broadcasts its own continuous score with no key, and the lab had never plugged into a real-world external feed (683 sonified a *financial* stream). **Could become a prototype that:** sits you inside the seismic present of the whole planet — each detection a struck resonant tone (magnitude→pitch/loudness/decay, depth→timbre, longitude→pan) + an expanding ripple on a world map, with a Gutenberg–Richter synthetic-swarm fallback so it always sounds → `712-seismic-room`.
- **Real-time audio-reactive WebGPU compute (2026)** — survey of current browser GPU practice (declarative WGSL compute, 20× CPU speedups, music-reactive WebGPU pipelines; iOS 26 bringing WebGPU "everywhere"). Confirms the renderer headroom for agent/particle sims in the browser, and seeded the WIDE fan-out's third explorer (a **Physarum** slime-mold agent-network choir, Jones 2010 / Sage Jenson "mold") — banked as IDEAS §465 `714-physarum-choir`, with the *real* WebGPU-compute port (50k–200k agents) noted as its deepening.
**This cycle's chain (research → build):** EMSC free real-time WebSocket + Florian Dombois's *Auditory Seismology* art practice → an **adult WIDE fan-out** of three ambitious explorers on three inputs (external-live-data / WebMIDI / autonomous-sim), with the lead direction being the lab's **first real-world external-live-data instrument** (`712-seismic-room`) — the literal realization of JURY #4. Banked: `713-shadow-duet` (first WebMIDI improviser, George Lewis *Voyager*) ⭐ + `714-physarum-choir`.

## §464 — 2026-06-18 (UTC) · research-first dive (cycle 464, KIDS, WIDE) — **JURY 2026-06-17 named three still-open kids gaps: #1 groove/rhythm not a cadence, #3 multi-user/two-player (still 0× across the whole window — the single most-starved register), #5 make a kid LAUGH. The question this cycle: what is the *current* cultural + scientific shape of "kids make a beat TOGETHER"?**
**Sources (date-checked this cycle, three WebSearches):**
- **Incredibox / Sprunki** — `sprunki.com` / `sprunki.org` ("Official 2026 Update", actively shipping new "phases" and mods this year). The dominant kids beat-making form right now is *stackable looping layers locked to one shared clock*: you drag a character onto the stage, it adds a loop that quantizes into the groove, the character bops, and the arrangement thickens — "sounds bounce, visuals pulse, every small action feels responsive." Surprising bit: this is a *2026-live* cultural object (kids are playing Sprunki *today*), and the lab has never built its core mechanic — a quantize-on-add loop-stacker — for kids. **Could become a prototype that:** lets two co-located kids each stack silly looping layers onto ONE shared beat (the first kids multi-user piece) → `711-kids-beat-buddies`.
- **arXiv:2504.03958** — *"Adaptation and synchronization — basic mechanisms in music performance."* The science under "playing together feels good": two performers lock to a shared pulse via continuous mutual *error-correction* — each nudges their timing toward the other, and the felt magic of an ensemble is this bidirectional entrainment, not either player alone. Leaning-foundational (Apr 2025) but it directly motivates the design choice that matters for kids: **quantize both players to ONE shared clock so they cannot help but lock in** — the togetherness is engineered, not demanded. **Could become a prototype that:** makes the shared-clock the instrument, so two 4-year-olds *are* a groove the instant they both tap.
- **arXiv:2508.10907** — *"Designing for Engaging Communication … Through Shared Music Experiences"* (Aug 2025). Shared music-making as a social-bonding interface (parent↔child framing). Reinforces that the unbuilt register isn't a new sound — it's the *second person*; co-play is the design space, and rhythm (vs. melody) is the lowest-floor way in for a non-reader.

**This cycle's chain (research → build):** Incredibox-2026 + entrainment-science → a **kids WIDE fan-out** of three groove/laugh explorers on three different inputs, with the lead direction being the lab's **first kids two-player shared-clock groove** (`711-kids-beat-buddies`) — directly answering JURY #1 + #3 + #5 at once.

## §463 — 2026-06-18 (UTC) · research-first dive (cycle 463, ADULT, DEEP) — **JURY 2026-06-17 #2 named the embodied-spatial / installation lane as the next off-glass swing ("push 677-presence-field to a cycle-2 … not another eyes-closed SVG"), and #4 said the median has gone comfortable — reach for the ceiling. So the question wasn't "what new register," it was "what cycle-2 capability makes 677 *massively bigger*?" 677 placed STATIC point-voices at joints. What does 2026 spatial-audio research say the felt-presence payload actually is?**
**Sources (date-checked this cycle, two WebSearches):**
- **"Effects of auditory distance cues and reverberation on spatial perception and listening strategies," arXiv:2505.18020** — the spatial impression is carried less by raw azimuth panning than by **distance cues + room reverberation** (a far source is quieter AND wetter; the reverb tail is the depth signal). The load-bearing idea: to make a body-driven spatial instrument *feel* like a room, couple **distance → attenuation + reverb-send**, not just pan. 677 only panned.
- **"Modeling and Driving Human Body Soundfields through Acoustic Primitives," arXiv:2407.13083** — the human body's *movement through space* is itself the signal shaping how sound distributes; pose + motion drive a spatial soundfield. Reinforces that an embodied-spatial instrument should let the body sculpt a *persistent* field, not just retrigger point sources.
- (context) **"Embodied Co-Creation with Real-Time Generative AI" (MDPI 2025)** — installations increasingly read full-body pose/presence as input to an *accumulating* generative state — the body builds something that persists, not a per-frame reaction.
**Why surprising / why it matters:** the obvious cycle-2 of 677 would be "more joints, prettier viz." The research says the real upgrade is two capabilities 677 lacked: (1) **distance + reverb as a felt axis** (near=present/dry, far=quiet/wet), and (2) **persistence / accretion** — voices the body LEAVES BEHIND, so the room is a *long-form spatial composition* genuinely different at minute 5 than at minute 1 (the 689/701-coral accreting-memory ceiling the jury keeps asking the lab to extend). That fuses the embodied-spatial lane (#2) with the reach-for-the-ceiling critique (#4) in one build.
**Could become a prototype that does X (today's build → chain, path a/b):** a DEEP fan-out of **one** concept — *moving your body leaves behind persistent HRTF voices that keep singing where you placed them, with distance→reverb, accreting into a luminous resonant architecture around your ears over minutes* — via 3 renderers: (a) `708-presence-cathedral` MediaPipe-Pose + three.js cathedral of light-pillars (the literal 677 cycle-2); (b) `709-presence-tide` zero-dependency webcam frame-diff + Canvas2D top-down room-map (the bulletproof no-CDN twin, the long-banked 678-presence-flow lane); (c) `710-presence-bloom` MediaPipe-Pose + **WebGPU** 60k-particle WGSL storm where each voice is an attractor (the Anadol-scale reach, Canvas2D fallback). → **Built this cycle; winner `710-presence-bloom`** (path a/b: arXiv:2505.18020's distance+reverb finding IS the felt axis; the accreting-memory layer is the cycle-2 capability over 677). 708/709 banked as IDEAS §463.

## §462 — 2026-06-18 (UTC) · research-first dive (cycle 462, KIDS, WIDE) — **The most jury-aligned move is the silliest one: the kids side has gone six-deep into "a real harmonic event behind a sensor" (the new pentatonic, per JURY 2026-06-17), and the "silly/funny" pole the lab once over-used is now at ZERO. So the freshest input isn't a new technique — it's a new *register*: comedy. What's alive in 2026 in playful sound that I could steal? Two threads stood out.**
**Sources (date-checked this cycle, two WebSearches):**
- **Toy Fair 2026 round-up (modernmom.com, 2026)** — the standout new category is *screen-free musical/sound toys* (e.g. the **Qubitunes** system) that reward physical play with sound, alongside a resurgence of sensory/stretch/ASMR toys. The signal: in 2026 the kids-toy market is rewarding **physical cause → satisfying sound**, not melody.
- **Ragdoll Physics Experiments (ragdollphysicsexperiments.com, 2026)** — modern ragdoll/physics toys drive their humor through *audio*: FMOD impact sounds with **dynamic pitch variation tied to collision velocity**, so "every collision sounds exactly as ridiculous as it looks." This is the load-bearing idea — **velocity → comedic pitch** is what makes a bonk funny, and it's trivially reproducible in Web Audio (vary an oscillator/blip's pitch by impact speed).
**Why surprising / why it matters:** the lab keeps treating "musical correctness" (scale-snap, voice-leading) as the value, but the live 2026 kids-toy register is the opposite — *gross, loud, velocity-pitched comedy SFX*. A bonk that pitches up when you hit it harder is funnier than any cadence.
**Could become a prototype that does X (today's build → chain):** a WIDE fan-out of three laugh-first kids toys that all reject harmony for comedy: (a) `705-kids-silly-voice` — record your voice, a googly monster replays it chipmunk/monster/backwards/robot (Talking-Tom voice-changer, zero scale); (b) `706-kids-tilt-bonk` — tilt a googly hero into bonkers whose SFX pitch rises with impact speed (the ragdoll-physics finding, directly); (c) `707-kids-mega-mouth` — make silly faces at the camera, a creature copies you and BURPS. The today's-research → today's-build chain is the velocity-pitched-comedy + voice-toy register from these 2026 sources.

## §460 — 2026-06-17 (UTC) · research-first dive (cycle 460, KIDS, WIDE) — **The kids side just opened its silly/groove pole (696-mouth-band) but is starved on the one register the jury says is 0× across the ENTIRE window: multi-user. The freshest, most jury-aligned move isn't another solo sensor toy — it's a SHARED piece where two kids' actions fuse, in real time, into one synchronized audio-visual groove neither made alone. The current frame for "sense each participant's state → fuse it into synchronized cross-modal feedback for a collaborative system" is articulated cleanly in this month's multisensory-intelligence vision, and the playful-multiplayer-browser-music lineage that makes it kid-legible is named and live.**
**Sources (date-checked this cycle, two WebSearches + one WebFetch):**
- **Paul Pu Liang, *"A Vision for Multisensory Intelligence: Sensing, Science, and Synergy,"* arXiv 2601.04563 (v3, 14 Jan 2026)** — argues that integrating multiple sensory modalities yields more robust, human-aligned systems, and specifically that audio-visual(-haptic) sensing lets a system *understand human state and interaction in shared environments*, supporting "more effective social and collaborative human-computer experiences" with *synchronized real-time feedback*. Read honestly: this is a **vision/position paper (~5 months old, not a <30-day drop)** — it is cited as the current conceptual frame for "sense each participant → fuse into one synchronized shared experience," not as a novel technique to implement.
- **The playful multiplayer-browser-music landscape (date-checked, still live):** **JAM with Chrome** (friends in different places play together in the browser) · **Plink** (a super-intuitive multiplayer music experience) · **Chrome Music Lab** (no-reading kid music toys) · **Toca Band** (Toca Boca — tap-a-character-to-add-a-part, the kid-legible co-play touchstone) · **Yamaha Tenori-on** (the lit 16-step grid sequencer) · **The Hub / League of Automatic Music Composers** (the pioneering networked ensembles — *send control data, not audio*). The web dive surfaced **no <30-day kids-multiplayer-audio drop** specific enough to claim ambition #1, so the build claims its floor honestly on #2 + #3.
**Why it's the right unlock:** JURY 2026-06-17 #3 begged for a **multi-user / two-player piece** ("still 0× across the entire window… social interaction is the genuinely unbuilt register, and it'd break the solo monoculture"), and #1/#5 banned the harmonic event on the kids side in favor of **groove, play, and laughter**. A two-phone shared-groove toy hits all three. The Hub's principle — *broadcast lightweight control signals, synthesize audio locally, phase-lock to one shared clock* — is the proven 694-pulse-ring architecture, ported to a kid-legible Toca-Band button interface. **Could become a prototype that:** lets two kids on two phones (same-origin `BroadcastChannel`, no server) tap silly monster sounds onto ONE shared 16-step loop, phase-locked to a broadcast beat-zero epoch, so their two patterns overlay into a groove neither made alone — and each monster squashes on the *other* kid's hits so the collaboration is *seen*. → **Built this cycle as `702-kids-monster-band`** (three.js bouncy monsters; path a — the dive's "synchronized shared-feedback" frame + the Hub/Toca lineage are its literal source). Two siblings banked: the bulletproof tilt-physics **`703-kids-boing-zoo`** ⭐ (jelly-animal bonks → slapstick rhythm; Canvas2D, zero deps/GPU/permission) and the off-glass **`704-kids-funny-face-party`** (front-camera motion → googly creature + FREEZE-dance game; Canvas2D).
**Freshness honesty:** the multiplayer-music lineage (JAM/Plink/Toca/Tenori-on/The Hub) is foundational, and the multisensory-intelligence paper is a Jan-2026 vision piece — **neither is a <30-day technique drop**; no fresh kids-multiplayer-audio paper surfaced this dive — so the winner claims its floor on **#2 (≥3 subsystems) + #3 (named refs)** = honest **2/5**; **#1 NOT claimed** (BroadcastChannel multi-user already exists in the lab at 319/334/341). The research role is path (a): the dive selected the *register* (multi-user, the jury's 0× ask), confirmed the named lineage that makes it kid-legible, and pinned the control-signal architecture; it did not inflate the ambition score.

---

## §459 — 2026-06-17 (UTC) · research-first dive (cycle 459, ADULT, DEEP) — **The lab's real-world-data-sonification thread has touched the market (683), the Wikipedia firehose (437), the sky (618), and the earth/seismic (613/233) — but never the one global real-time signal that is purely a portrait of collective human *making*: the worldwide stream of code being written. The jury's #4 explicitly invited it ("683's WebSocket engine could sonify a non-market live stream — transit, seismic, language-usage — for free"). The freshest, most reliable, keyless such stream is the GitHub public-events firehose, and the canonical artistic lineage for sonifying a live collaborative-creation firehose already exists and is named.**
**Sources (date-checked this cycle, two WebSearches):**
- **Hatnote "Listen to Wikipedia"** (listen.hatnote.com, Stephen LaPorte & Mahmoud Hashemi, ongoing since 2013) — the canonical, still-live sonification of a global collaborative-creation firehose: every Wikipedia edit becomes a bell (addition) or string (subtraction), scaled by edit size, with a soft choral bloom for new users. It is the direct conceptual ancestor of "sonify the world's act of making, gently, as ambient texture."
- **github.audio** + the broader data-sonification field (Wikipedia *Data sonification*; the *interactive-sonification* GitHub org's `mesonic`/`pya` tooling; Björk's *Kórsafn* weather-driven AI installation 2020; **Brian Foo**'s *Data-Driven DJ* / *Two Trains*; **Ryoji Ikeda** *data.matrix*; **Refik Anadol** planetary data-as-light) — the named lineage for live-data → generative-ambient and data-as-spectacle. The web dive surfaced **no <30-day drop** specific enough to claim #5; the register's frontier is foundational, so the build claims its floor honestly on #2 + #3.
- **Feasibility check (the unlock):** `https://api.github.com/events?per_page=100` is an **unauthenticated, keyless, CORS-open** (`Access-Control-Allow-Origin: *`) read-only GET — every push / star / PR / issue / fork worldwide, 60 req/hr per IP. From a visitor's browser that is a free, no-secret, no-api-route, no-guard live firehose; batch-poll ≤1/~78s + meter events out + a synthetic-first generator → it runs forever offline. This is the proven 683-world-pulse / market-tide robustness pattern, ported to a free stream.
**Why it's the right unlock:** JURY 2026-06-17 #4 begged the lab to **extend the ceiling builds rather than start "a sixth tonal-tension toy,"** naming 683's live-data engine + a non-market stream as a free path; #1/#5's "groove/texture not a cadence" technique-ban also pushes away from harmony-as-subject toward a living texture. **Could become a prototype that:** sonifies the live global GitHub firehose — event type → timbre, repo-hash → pitch in a drifting D-Dorian mode, aggregate rate → an evolving drone — as the planet's ambient act of creation. → **Built this cycle as `699-world-forge`** (a three.js rotating Earth alive with geo-located event sparks; path a — the dive's firehose + lineage are its literal source). Two siblings banked: the long-form **accreting-memory** reef (`701-living-coral` ⭐, WebGPU/Canvas2D — the directest "reach for the ceiling," echoing 689) and the **Ikeda austere data-rain** (`700-commit-rain`, Canvas2D).
**Freshness honesty:** the lineage (Hatnote, github.audio, Foo, Ikeda, Anadol) is foundational, **not a <30-day drop** — no fresh paper surfaced this dive — so the winner claims its floor on **#2 (≥3 subsystems) + #3 (named refs)** = honest **2/5**; **#1 NOT claimed** (live-data sonification already exists in the lab at 683/437/618). The research role is path (a): the dive selected the data source + confirmed the named lineage + verified the keyless-CORS feasibility that makes the whole fire free and unattended-safe; it did not inflate the ambition score.

---

## §458 — 2026-06-17 (UTC) · research-first dive (cycle 458, KIDS, WIDE) — **The kids side has gone uniformly solemn (JURY 2026-06-17: six straight tension-and-resolution machines; the silly/funny pole is now 0×). The freshest move isn't another harmonic trick — it's to make the *subject* groove, play, and laughter, and to take the input off the glass and into the body (mouth, shake, dance). The cleanest technical backbone for a funny, groove-first kids toy is real-time VOCAL PERCUSSION: a child's mouth noise → onset + spectral classification → a silly drum in a looping beat.**
**Sources (date-checked this cycle, two WebSearches):**
- **Incredibox** (incredibox.com) — the canonical *playful* beatbox-as-toy: drag silly sounds onto characters who loop them into a groove, instantly readable, joyful, no reading required. The cultural touchstone for "a kid builds a groove by being silly," and the direct vibe reference for a kids vocal-groove machine.
- **Vocal-percussion / beatbox classification literature** — Hazan & Stowell, *"Delayed Decision-making in Real-time Beatbox Percussion Classification,"* Journal of New Music Research **39(3), 2010** (defer the final class until just after triggering, to balance 12–35 ms low-latency response against reliable kick/snare/hat labelling) + Mehrabi et al., *"A New Dataset for Amateur Vocal Percussion Analysis,"* Audio Mostly **2019** (the **AVP dataset** — 9,780 utterances from 28 *amateur* beatboxers, onset-annotated kick/snare/closed-hat/open-hat). Together they give a robust, ML-free recipe: an energy-onset gate + a spectral-centroid/low-band bucket → 3–4 silly drum voices, fast enough for a 4-year-old's "boom / tss / pop / brrr" to feel instant.
**Why it's the right unlock:** JURY 2026-06-17 banned the harmonic-event/voice-leading technique on the kids side for a week (8× in window, 6× on kids — the new pentatonic), named the silly/funny pole at **0×** ("make a kid LAUGH, not hush"), and flagged that the danceable/groovy/**body** register got built once (652-kids-dance-band) and was abandoned ("the kids side fled rhythm for harmony"). A funny, groove-first, embodied toy lands all three. **Could become a prototype that:** a child makes mouth noises → onset+centroid classification drops each into a looping silly beat a goofy creature bops to. → **Built this cycle as `696-kids-mouth-band`** (path a — the vocal-percussion recipe is its literal classifier), inside a WIDE fire of three off-glass silly/groove explorers (mic / shake-motion / camera-dance) so the comedy register is attacked from three embodied directions at once.
**Freshness honesty:** the beatbox-classification papers are foundational (2010/2019), not a <30-day drop, so the winner claims its floor on **#2 (≥3 subsystems) + #3 (named refs: Incredibox; Hazan & Stowell 2010; AVP dataset 2019)** = honest 2/5, with a plausible **#1** (first real-time vocal-percussion onset classification in the lab — grep-checkable). The research role is path (a): the recipe bends the build's classifier, and the jury's "make a kid laugh / groove not cadence / get off the glass" directive sets the whole fire's diversity tags.

---

## §457 — 2026-06-17 (UTC) · research-first dive (cycle 457, ADULT, DEEP) — **The 2026 collaborative-web-music stack has converged on one design principle that is exactly the unlock for the lab's most-starved register (multi-user, 0× in the last 15): broadcast lightweight *control signals*, not audio; synthesize locally on every client; lock everyone to one shared metronome grid. That is the architecture of a serverless, same-origin BroadcastChannel groove room — and the jury is begging for groove + multi-user, not another cadence.**
**Sources (date-checked this cycle, two WebSearches):**
- **IEEE 2025–2026** — *"Real-Time Collaborative Music Creation on the Web: exploiting Web Audio Modules for Interactive Performance and Composition"* (ieeexplore.ieee.org/document/11325761) + **CHI 2026** *"Plug the Guitar In Before You Put the Headset On"* (dl.acm.org/doi/full/10.1145/3772318.3793871) + the live **sequencer.party / Web Audio Modules** ecosystem: collaborative web-music now standardizes on **CRDT state synchronization** for a shared, conflict-free score/grid plus **Web Audio Modules** for local DSP. State is replicated; sound is made on each client.
- **Networked-music-performance (NMP) practice, 2026** — the *"Low Latency Music Collaboration: Your 2026 Guide"* (soundbridge.io) crystallizes the working pattern: **control signals (MIDI-style messages) travel the network; audio is synthesized locally; a shared metronome + beat-quantization lock all participants to one timing grid.** (JackTrip/LoLa's audio-streaming lineage is the *opposite*, latency-bound approach — the control-signal pattern is what survives commodity-latency transports.)
**Why it's the right unlock:** the lab has built multi-user before (319-hub-score, 334-pass-the-song, 341-star-pair) but it is **0× across the JURY 2026-06-17 window** — the single most-emphasized standing gap ("social interaction is the genuinely unbuilt register"), and the jury also banned the harmonic-event/voice-leading technique (the new monoculture) and the hushed vibe, begging for **groove/rhythm/play**. The control-signal-not-audio + CRDT-grid + shared-epoch pattern is *exactly* what a BroadcastChannel groove room needs. **Could become a prototype that:** several people share one rotating clock and each drops percussion beads at angular positions on different subdivisions → emergent group polyrhythm/phasing, locally synthesized, conflict-free, locked to one broadcast epoch. → **Built this cycle as `694-pulse-ring`** (path a — the dive's control-signal pattern is its literal architecture). Two siblings banked: a 16-step collaborative band-room (`693-ensemble-floor`) and an accumulating turn-taking jam (`695-trade-fours`).
**Freshness honesty:** the *venues* are current (IEEE/CHI 2026, sequencer.party live) but the core idea (control-signals over the wire, NMP, CRDTs) is foundational — so the build claims its floor on **#2 (≥3 subsystems) + #3 (named refs: The Hub / League of Automatic Music Composers; Steve Reich phasing; West-African clave timeline)**, an honest **2/5**; **#1 NOT claimed** (BroadcastChannel/multi-user already exists in the lab). The research role is path (a): the control-signal principle bent the build's architecture, not its ambition score.

---

## §456 — 2026-06-17 (UTC) · research-first dive (cycle 456, KIDS, WIDE) — **The freshest move for the over-mined kids harmonic-event lane is not another on-glass harmonic trick but to put the harmony back in the *body*: recent music-cognition work shows a child's perception of a piece's emotional/expressive character is modulated by their own (and observed) body movement — so let the child's *movement energy itself* drive a real major↔minor mode-morph, eyes off the glass.**
**Sources (date-checked this cycle, four WebSearches):**
- **Music-cognition / embodied perception** — "The Influence of Body Movements on Children's Perception of Music with an Ambiguous Expressive Character" (PMC3554646) + the 2025 pitch-perception-in-school-aged-children work (resolved vs. unresolved harmonics, PMC11789513): children track harmonic structure and their *perceived expressive character* shifts with body movement. The named pedagogical lineage is **Reggio Emilia** embodied music education ("musical understanding develops through movement," KIDS.md). → became the harmonic brain of the WIDE winner: movement/voice energy → continuous mode-morph (bend the 3rd M3→m3, crossfade the 6th M6→m6 over a fixed root+fifth drone) = an audible major↔minor flip with **zero pentatonic scale-snap**. Research→build chain is **same-day**.
- **Three.js 2026 / WebGPU creative-coding frontier** — "What's New in Three.js (2026): WebGPU, New Workflows & Beyond" (utsubo.com) + the WebGPU "it's the present, not the future" roundups: the live velocity-feedback-texture technique (particles write velocity to a shared field and read it back, spawning organic motion with no keyframes) and **MPM (Material Point Method)** particle+grid fluids are now browser-feasible. *Could become a prototype that:* a kids fluid where the flow's behaviour encodes the current chord. (Banked direction; not built this cycle.)
- **Collaborative touchscreen music** — **RoboJam** (Martin, Glette, Nygaard, Tørresen) + **ToCoPlay** + early-childhood "group synchrony / joint attention" literature: side-by-side music-making raises musical+social bonding. → seeded the banked `692-kids-two-friends` (two-voice interval consonance + voice-leading resolution).
- **Could become a prototype that does X:** an off-glass, eyes-off kids piece where *how the child moves their body* (tilt + motion + mic energy) is the only control, steering a continuous major↔minor mode-morph — calm = sun, wild = storm. → built this cycle as **`690-kids-weather-hands`**. The off-glass framing also directly attacks JURY 2026-06-16 #2 (13/15 are screen renders).
**Freshness honesty:** the embodied-perception literature is foundational (not a <30-day drop), so the winner claims **#5 only loosely**; its honest floor is **#2 (≥3 input subsystems fused) + #3 (named refs: embodied music-cognition / Reggio Emilia; Eno generative ambient)** = 2–3/5. The three.js-2026 + RoboJam findings ARE current (2026) and seed the two banked siblings.

---

## §455 — 2026-06-17 (UTC) · research-first dive (cycle 455, ADULT, DEEP) — **The live "real-time audio-reactive AI art" stack (TouchDesigner + StreamDiffusion) is the contemporary frontier of image-gen-inside-AV — and the surprising move is to invert its per-frame reflex into a slow, long-form piece that dreams ONE deliberate image per multi-minute movement and KEEPS them as memory.**
**Sources (date-checked this cycle, two WebSearches + an arxiv probe):**
- Derivative (TouchDesigner) community tutorial **#216 — "TouchDesigner & StreamDiffusion: Real-Time Audio-Reactive AI Art"** (derivative.ca) + the **2025 ACM IMX** paper *"An AI-driven Music Visualization System for Generating Meaningful Audio-Responsive Visuals in Real-Time"* (dl.acm.org/doi/10.1145/3706370.3727869). The state of the art: img2img latent diffusion (StreamDiffusion) driven *per frame* by audio features isolated into frequency bands (kick/snare/bass band-energy → noise/movement parameters), so the picture re-renders in lockstep with the beat.
- Context: WebGPU is now "the present, not the future" for browser creative-coding (webgpuexperts.com May-2025 roundup, lisyarus particle sims) — the substrate for porting these AV pipelines into the browser exists.
**Why it's surprising / the inversion:** every reference treats AI-image as a *reactive surface* — fast, per-frame, disposable, in lockstep. Resonance's vibe is immersive/contemplative/personal, the opposite of a strobing reactive surface. The hook: don't chase per-frame paid diffusion in the browser — go the OTHER way. **Could become a prototype that:** runs a slow generative journey and at each multi-minute *movement* boundary mints ONE deliberate flux/schnell image as a remembered "chapter," accumulating a navigable 3D gallery of its own past — long-form *memory* rather than reactive *surface*. → **Built this cycle as `689-dream-chapters`** (the slow inverse of StreamDiffusion). Two adjacent hooks banked: the audio→image→**audio** closed loop with color-feedback (`687-latent-oracle`) and the **re-sonified image-as-instrument** (`688-spectral-seance`, Ikeda/UPIC/ANS lineage). All three keep a procedural fallback so they run with zero network/key.

---

## §454 — 2026-06-17 (UTC) · research-first dive (cycle 454, KIDS, WIDE) — **A fresh (<30-day) chord-generation paper turns out to describe, almost line for line, the architecture of a child-driven reharmonizer. The jury's named kids local-minimum is the "pentatonic / no-wrong-notes" crutch, and §452 already found the inverse — give full chromatic freedom and let the HARMONY chase the note. This cycle's dive found the *engineering* of that idea shipped this month: He/Li/Sun/Huang decompose chord generation into RETRIEVE → EDIT → RERANK. That is exactly the loop a "no wrong note, ever" kids instrument needs: the child's tapped pitch class is a hard constraint; RETRIEVE candidate chords that could contain or warmly frame it; EDIT/score by music-theoretic fit (chord-tone > extension > clash penalty); RERANK by minimal voice-leading so the pads glide rather than jump.**
**Sources (date-checked this cycle, two WebSearches + one WebFetch):**
- **He, Li, Sun & Huang — "A Decomposed Retrieval-Edit-Rerank Framework for Chord Generation"** — arXiv:2605.07489, **submitted 2026-05-08** (verified via arxiv.org/abs/2605.07489, ICMR '26 Amsterdam). The decomposition "establishes stylistically plausible options (retrieve), enforces music-theoretic constraints through minimal adjustments (edit), and selects among feasible candidates (rerank)," giving explicit control over the diversity↔feasibility balance vs. end-to-end models. It is symbolic-domain and not itself real-time, but the decomposition ports directly to a tiny client-side reharmonizer.
- **Supporting (older, foundational):** pitch-perception-in-children work (resolved vs. unresolved harmonics, 2025) confirms school-aged children track harmonic structure; the psychophysical "tension and resolution" basis of harmony (Helmholtz roughness; Meyer; Huron) underpins the sibling explorers. Sony CSL *FlowComposer* reharmonization (§452) remains the named lineage for "harmony chases the melody."
- **Could become a prototype that does X:** a kids instrument with the FULL 12-note chromatic palette where every tap is reharmonized live — a "wrong" note simply becomes the color of a freshly retrieved chord. → became **`684-kids-every-note`** (`harmony.ts` implements `scoreFit` [retrieve+edit] + `voiceLeadingCost` [rerank] + a stay-bias). Chain `today's research → today's build` is **same-day** and visible.
- **Freshness honesty:** the lead paper IS a <30-day drop (2026-05-08), so the build legitimately claims **#5 (recent-research, last 14 days bind is comfortably met)** alongside #2 (≥3 subsystems) + #3 (named ref). The two sibling explorers (685 mode-glide, 686 suspension) rest on foundational refs only (Cooke; Bach/Helmholtz/Meyer/Huron) → honest 2/5 each; 684 is the 3/5 winner.

## §453 — 2026-06-17 (UTC) · research-first dive (cycle 453, ADULT, DEEP) — **The lab's real-world-data-sonification thread is deep but lopsided: it has sonified the SKY (`618-solar-organ`, live NOAA space weather), the EARTH (`233-earth-pulse`, `613-seismic-choir`), a historic CHESS game (`649-chess-duet`), and the live WIKIPEDIA edit firehose (`437-wiki-pulse`) — but never the one real-time signal that is most purely a portrait of *collective human emotion*: the live financial market. And the technical unlock is a transport the lab has literally never opened — a `WebSocket` (grep-clean: zero `new WebSocket` / `wss://` across all ~680 prototypes; every prior live feed used one-shot `fetch` or SSE/`EventSource`). A keyless public trade firehose (Binance `wss://stream.binance.com:9443/ws/btcusdt@trade`) streams every executed trade — price, size, and crucially the *aggressor side* (buyer- vs seller-initiated) — tens of times a second, a real-time index of the world's fear and greed. The surprising reframe for a meditation app: don't render a trading screen — render the market as WEATHER you witness (a tide, a nebula, a data-rain), where collective buy-pressure warms toward consonance and sell-pressure darkens toward a tension that wants to resolve.**
**Sources (date-checked this cycle, four WebSearches):**
- **Brian Foo — "Data-Driven DJ" / "Two Trains: Sonification of Income Inequality on the NYC Subway"** (datadrivendj.com) — the canonical named reference for *data → music as witness*: the quantity and dynamics of a track's instruments correspond to the underlying data (median household income per subway stop). The browser-feasible reduction here: map live trade aggressor-side → chord color, trade size → bell register/loudness, rolling momentum → consonance↔tension, volatility → grain/foam density, over an always-present drifting modal bed.
- **Ryoji Ikeda — "test pattern" / "data.matrix"** — the pure-sonification-of-data aesthetic (data as surgical white-on-black spectacle) — the named anchor for the austere/clinical approach of the DEEP fan-out (`685-ledger-lines`).
- **Wikimedia EventStreams + the broad 2026 WebGPU-immersive-web data-art front** (verified active this year) confirm live-stream → audio-visual is a current, living practice — but also that the lab's own coverage (solar/seismic/chess/wiki) leaves the **market firehose** conspicuously un-built, and the **WebSocket transport** entirely un-used.
- **Could become a prototype that does X:** an ADULT, long-form, stateful meditation where a live market WebSocket drives a never-repeating weather system — momentum bends the harmony between tension and release over minutes, so the piece is genuinely different at minute 5 than minute 0. → **DEEP target this fire** (3 approaches: `683-world-pulse` WebGPU data-nebula / cosmic-transcendent · `684-market-tide` Canvas2D breathing sea / tender-oceanic · `685-ledger-lines` Canvas2D Ikeda data-field / clinical), each with a **synthetic-market fallback** so a silent 06:30 glance is alive even when the socket is blocked.
- **Honest caveat:** the named refs are foundational (Data-Driven DJ ~2016; Ikeda *test pattern* ~2008) and no clean <30-day frontier paper anchors the music-from-markets idea — so **#5 (recent-research) is NOT claimed** on freshness. The dive's role is the research-first rule's **path (a)**: it audited the lab's data-sonification coverage, found the market firehose + WebSocket as the genuine gaps, and bent the build there. Ambition rests honestly on **#1 (never-used technique: first `WebSocket` live-stream input, grep-verified)** + **#2 (≥3 subsystems)** + **#3 (named refs: Brian Foo · Ryoji Ikeda · Refik Anadol)**.

## §452 — 2026-06-17 (UTC) · research-first dive (cycle 452, KIDS, WIDE) — **The jury's named kids local-minimum is the "pentatonic / no-wrong-notes" crutch (6× of the last 15 kids pieces) — and the literature points at the *inverse* of that crutch. The usual trick prevents wrong notes by RESTRICTING the scale (five safe pads). But the deeper musical truth is that consonance is CONTEXTUAL — a single note is neither right nor wrong on its own; its quality depends entirely on the chord beneath it. So the bigger move for a child instrument is not "remove the wrong notes" but "give full chromatic freedom and let the HARMONY chase the note" — real-time reharmonization, the practice automated by Sony CSL's FlowComposer (reharmonize a melody "in the style of Bill Evans / Michel Legrand"). The other firm ground for a 4yo (carried from §448/§450): they perceive *closure and its denial* (cadence) and *change itself* well before they map minor→sad (~age 7).**
**Sources (date-checked this cycle, three WebSearches):**
- **Sony CSL *FlowComposer* / automatic reharmonization lineage** — corpus/style-based reharmonization of a fixed melody (the canonical demo: "Yesterday" reharmonized in the style of Bill Evans / Michel Legrand). The browser-feasible reduction: a small palette of lush candidate chords (diatonic + borrowed colors), each tapped pitch scored for fit, the best-fitting chord chosen and voice-led under the note in real time. The principle it operationalizes — *consonance is contextual* — is the directest theoretical answer to the jury's "let a kid meet a wrong note and discover it isn't scary."
- **"Music and Rhythm as Promising Tools to Assess and Improve Cognitive Development in Children: A Scoping Review" (Acta Paediatrica 2025, PMC12420879)** — 23 studies; music/rhythm engagement positively affects executive function, attention, self-regulation. Supports the embodied/sensorimotor framing (rocking, tilting) over button-pressing for a young child, and the developmental caution that *active embodied participation* drives the benefit.
- **Carried-forward anchor (§448/§450, `[older, foundational]`):** 3–4yos don't reliably map minor→sad until ~age 7–8; the perceptible harmonic events at that age are (a) *change itself / a moving root* and (b) *closure & its denial* (cadence). → ordered curation: cadence (680) sits on firmer 4yo ground than the chromatic-reharmonizer's subtler payload (681) and far firmer than the mode-glide (682, which is therefore framed 6+/family).
- **Could become a prototype that does X:** (a) a kids instrument with **full chromatic freedom where the band reharmonizes underneath to make every note consonant** — the literal inverse of the pentatonic crutch (→ built as `681-kids-every-note`, banked winner-adjacent); (b) an **off-glass** lullaby where rocking the tablet drives a **deceptive→authentic cadence** so the child *feels* a musical sentence finish (→ **shipped `680-kids-night-boat`**); (c) a 6+/family **continuous major↔minor mode-glide** (→ `682-kids-two-moons`, banked).
- **Honest caveat:** no clean <30-day frontier drop surfaced for the kids-harmony register this cycle (FlowComposer is foundational ~2016; the scoping review is 2025; the developmental anchor is foundational) — so **#5 is NOT claimed** on freshness. The dive's role is the research-first rule's **path (a)/(c)**: it bent the fire away from the banned scale-snap toward *embracing* the wrong note (reharmonization) and toward the developmentally-firm cadence, and it consumes the recent §450 banked kids seeds (676→680, 674→682).

## §451 — 2026-06-17 (UTC) · research-first dive (cycle 451, ADULT, DEEP) — **The lab's single biggest untouched first, named by THREE consecutive juries, is the *embodied-spatial / room-presence* piece — and the research front says the payload of such a piece is not the visual but the *spatialization itself*: placing voices in 3D around the listener via HRTF/panning measurably raises the felt SENSE OF PRESENCE. The surprising move is to make where-you-stand-in-the-room literally place the sound in the room — body POSITION → sound POSITION (binaural PannerNode field) — so the instrument you play is the acoustic geometry of the space, not a control on the glass.**
**Sources (date-checked this cycle, two WebSearches):**
- **arXiv:2601.22082 (Jan 2026) — "Auditorily Embodied Conversational Agents: Effects of Spatialization and Situated Audio Cues on Presence and Social Perception."** Finding: spatializing an agent's sounds (binaural placement) + situated Foley that reflects the agent's position in the environment measurably increases the user's sense of presence and social perception. The generalizable principle: *spatial placement of sound is itself the immersion mechanism*, not a garnish on top of one.
- **arXiv:2412.05486 (Dec 2024) — "A Scene Representation for Online Spatial Sonification"** — builds an incremental 3D scene from depth sensing and collapses it into a compact 360° auditory representation in real time. The browser-feasible analogue: read the body's position from a camera (no depth sensor needed) and sonify it as a 360° field around the listener. Reinforced by **"Sounding Bodies" (arXiv:2311.06285)** — modeling 3D spatial sound from body pose — and the **IRCAM/ISMM** movement-*qualities* sonification lineage already cited in §442.
- **Could become a prototype that does X:** an ADULT, off-the-couch **room-presence** instrument — step back from the screen and your body in the camera frame conducts a *spatial* ensemble: each voice is placed via Web-Audio `PannerNode` (HRTF) at the screen-x / screen-y / implied-depth of where you (or your limbs) are, so moving through the room sweeps the ensemble around you; presence/spread/motion drive swell, register and reverb. The spatialization IS the instrument. → **DEEP target this fire** (3 approaches to body-sensing: MediaPipe Pose skeleton, pure camera frame-diff motion-field, MediaPipe selfie-segmentation silhouette), realizing the long-banked `442-body-orchestra` seed.
- **Honest caveat:** the spatialization→presence principle is from a Jan-2026 paper (~5 months, NOT a <30-day frontier drop) and the depth-sonification paper is Dec-2024 — so **#5 is NOT hard-claimed** on freshness. Its role is the research-first rule's **path (a)/(c)**: the dive directly bends the build (spatialization is the payload, not the viz) AND it consumes the banked `442-body-orchestra` seed, while landing the jury's #1 standing untouched ask. **#1 (never-used technique) NOT claimed via MediaPipe** — the lab has used MediaPipe pose/hands/segmentation many times on the KIDS side (217/287/295/419/444/582/617). The genuinely-open frontier is the *register*: an ADULT installation / room-presence piece with **spatialization-as-instrument** has 0× builds. Ambition rests honestly on #2 (≥3 subsystems) + #3 (named refs: the spatialization-presence paper + Myron Krueger *Videoplace* + Refik Anadol room-scale presence works).
...
## RESEARCH §461 — 2026-06-17 (UTC) · adult cycle dive · the audio→image→AUDIO closed loop (cross-modal round-trip)

- **arXiv:2512.08951 — "AI Co-Artist: A LLM-Powered Framework for Interactive GLSL Shader Animation Evolution" (Dec 2025)** — https://arxiv.org/html/2512.08951 . An LLM *generates and evolves* a population of distinct audio-reactive GLSL fragment shaders; the human acts as a **curator**, selecting the visually compelling ones, and the system breeds the next generation from those choices. **Why it's surprising:** it inverts the usual "one artist hand-tunes one shader" loop into a *generate→curate→evolve* loop — which is structurally the same choreography the dream lab itself now runs (planner→parallel makers→critic). It also points at a frontier the lab hasn't touched: the visual layer as a *searchable population*, not a fixed program.
- **The image-as-spectrogram lineage (foundational anchor, drives today's build):** spectrogram↔waveform synthesis (the convolutional spectrogram→waveform line) makes the *return trip* — turning a 2D image back into sound by reading it as a time×frequency grid — a stable, decades-deep technique (Xenakis **UPIC** 1977, the Russian **ANS synthesizer** 1958, MetaSynth, the modern neural spectrogram-vocoder stack). Tagged `[older, foundational]` per the freshness carve-out: included because it *enables* this cycle's build, not as a last-30-days claim.
- **Could become a prototype that** (and did, this cycle): a generative sound dreams an austere data-image (flux), then the image **becomes the next instrument** — re-sonified column-by-column as a spectrogram while a scan-line sweeps it, so you literally see and hear the picture played. Shipped as **`704-spectral-seance`**. (The fresher AI-Co-Artist *evolve-a-shader-population* hook is banked for a future "the visuals are a searchable population" piece — not built today.)
- **Research→build chain (mode DEEP — ONE concept [the music dreams a picture and the dream changes the music], 2 technical approaches to closing the loop; ship the strongest):** path (c) — the build consumes the closed-loop seeds **687-latent-oracle + 688-spectral-seance**, both banked from **RESEARCH §455**'s same-day AI-pipeline dive that produced ceiling-build 689; JURY 2026-06-17 #4 ("extend the ceiling builds, not a sixth tonal-tension toy") selected this concept over a fresh build. Approaches: 703 closes via **color**→synth (Anadol soft nudge) vs 704 closes via **literal re-sonification** (Ikeda image-as-instrument). Winner + floor/diversity recorded in STATE §461 after curation.

---

## §467 — 2026-06-18 (UTC) · ADULT dive: interactive machine co-improvisation (the "duet partner's mind")

- **arXiv:2502.00023 — "Musical Agent Systems: MACAT and MACataRT" (Feb 2025)** — https://arxiv.org/pdf/2502.00023 . Two human-in-the-loop co-creative musical agents: **MACAT** (real-time generation/analysis/curation) and **MACataRT**, which builds a *creative space* for human-AI co-improvisation via **concatenative synthesis** — it answers by retrieving and recombining grains from an audio *corpus* rather than synthesizing tones. **Why it's surprising/useful:** it reframes "the machine's reply" as a *retrieval* problem over a real recording, not a synthesis problem — so a duet partner can literally be made of *someone's actual playing*. Tagged `[recent — Feb 2025]` (>30d but the freshest concrete co-improv-agent paper; the concatenative-co-improv lineage Diemo Schwarz **CataRT** + IRCAM **OMax/Somax2** is the `[older, foundational]` anchor).
- **Somax 2 (IRCAM) / Pachet Continuator (2002) / George Lewis *Voyager* (1980s–)** — the named lineage of *listen → infer style → answer in turn*. Somax2 = real-time machine-listening + stylistically-coherent response; Continuator = a variable-order Markov model that learns *your* phrase vocabulary live and continues in your style; Voyager = a rule-based interactive improviser that "trades" with the soloist. These are the three distinct *minds* a call-and-response partner can have: **rule-based taste**, **learned style**, **corpus retrieval**.
- **Could become a prototype that** (and did, this cycle): **Shadow Duet** — you play a phrase (MIDI / keyboard / on-screen keys) and a machine improviser **answers in the gaps**, trading fours like a partner with taste. Directly serves Karel-the-pianist + the jazz-responsive *alternate journey-arc* he asked for + live-performance fitness. Built `mode: DEEP` — ONE concept, the three minds above as three parallel approaches: **716-shadow-duet** (Voyager-style heuristic taste), **717-duet-mirror** (Continuator-style live style-learner), **718-duet-paths** (MACataRT-style concatenative shadow of Karel's *real* Welcome Home piano). Ship the strongest; bank the rest.
- **Honesty correction to the banked `713-shadow-duet` seed:** its "lab's FIRST Web MIDI" claim is **false** — `navigator.requestMIDIAccess()` already appears in 4-operator, 291-harmonograph, 365-cadence-ladder, 375-tempo-canon, 391-resilient-accompanist, 483-two-rooms-apart, 486-third-room. MIDI is not a #1. The genuine novelty is the *responsive jazz call-and-response improviser arc* + (for 718) answering with his real recording. Floor claimed honestly at #2+#3.

## §468 — 2026-06-18 (UTC) · KIDS dive (cycle 468, WIDE) — **468 % 2 == 0 → kids. The binding directives are JURY 2026-06-17 #2 ("the next off-glass swing should be embodied-spatial/installation/haptic, NOT another eyes-closed glow-screen") + #5 ("MAKE A KID LAUGH — the silly pole is 0×, the kids side has gone uniformly solemn") + #1 ("groove/play, NOT a cadence/voice-leading — banned on kids for a week"). The question: what shipped in the last 30 days in embodied/movement interaction, and what's the named-reference grounding for whole-body → sound for children?**

- **Ghost Arcade — open-source projection-mapping / VJ software (live, June 2026)** — https://ghostarcade.live/ . Its newest layer type runs **WebGPU compute shaders directly, zero-copy from compute output into the compositor**, with audio-reactive parameter modulation, 316+ GPU shaders, Spout/Syphon, MIDI clock, and a mobile companion. **Why it's surprising:** it's the first *consumer/VJ-grade* tool treating a WebGPU compute pass as a first-class, audio-reactive composite layer with no readback round-trip — exactly the TouchDesigner-TOP paradigm Karel keeps asking to port to the browser, now with a working reference implementation. Tagged `[recent — June 2026]`. **Could become a prototype that:** a WebGPU-compute particle/feedback field whose simulation parameters are modulated live by Karel's piano FFT (an adult/installation fire — the genuine port of the `714-physarum-choir` WebGPU deepening + the JURY-#2 embodied-spatial lane). *Banked as an adult seed; not this cycle's kids build.*
- **BodyBeats — whole-body musical interfaces for children (foundational)** — research mapping children's whole-body movement qualities to sound in real time via **Rudolf Laban's Theory of Effort** (weight/space/time/flow); kids "play" by mimicking objects/characters and the movement *quality* drives the synthesis. Cross-referenced with the Kinect skeletal-tracking → note-generation lineage (velocity/acceleration/limb-position → pitch/density). Tagged `[older, foundational]` — it's the academic grounding the lab's off-glass kids pieces have lacked a citation for. **Why it matters here:** it's the named reference that makes "stomp in front of the camera → percussion" a principled mapping (movement effort → sonic gesture), not an ad-hoc gimmick.
- **This cycle's chain (research → build):** path **(c)** — consumed the §466-banked `715-kids-stomp-zoo` seed (itself born of RESEARCH §466's VTech-2026 "your-movement-is-the-controller" thesis) as the lead brief, now grounded by BodyBeats/Laban. Ran a kids **WIDE** fan-out of three laugh-first explorers on three *embodied non-glass inputs* so they can't collapse into one another: **SEE** (`719-kids-stomp-zoo`, webcam whole-body stomp → tuba-parade) · **SHAKE** (`720-kids-shake-pop`, devicemotion → fizzy-bottle build-and-erupt) · **BLOW** (`721-kids-blow-balloon`, mic-breath → balloon hum + raspberry). All three clear every JURY rolling-15 ban (groove/joke not cadence · embodied not touch-on-glass · Canvas2D not glow/WebGL2 · silly not hushed). **Winner: 719** (most embodied = truest answer to JURY #2's named embodied-spatial lane, + the standing RESURRECT-FIRST seed). Banked 720 ⭐ + 721 as IDEAS §468.

## §469 — 2026-06-18 (UTC) · ADULT dive (cycle 469, DEEP) — **the navigable corpus map: "fly inside your own recording."** Binding directives: JURY 2026-06-18 #3 ("stop banking ceilings and walking away — push 710/704 to a cycle-2, reward DEPTH over breadth"), #1 ("ban Canvas2D — it's the new 9× monoculture; 710 proved a WebGPU-compute stack degrades cleanly, port that"), #5 ("use Karel's REAL music — it's in only 1 of the last 15"). The question: what's the named-reference + recent grounding for turning his real recording into a *spatial map you navigate to re-sound it*?

- **Diemo Schwarz · CataRT — real-time corpus-based concatenative synthesis (foundational anchor; drives the build)** — https://www.researchgate.net/publication/224927716 . The recording is segmented into grains, each analyzed for descriptors (energy, brightness/spectral-centroid, pitch); grains are laid out in a **2D descriptor space** and a cursor *navigates* that space, selecting + concatenating the nearest grains in real time. Tagged `[older, foundational]` per the freshness carve-out — it's the technique the build *is*, not a last-30-days claim. The point: "the machine's voice = retrieval over a real recording" (the same reframing §467 found in MACataRT, now turned from a duet-partner into a navigable *place*).
- **TU-Berlin (DAFx 2025) — "Pitch-Conditioned Instrument Sound Synthesis from an Interactive Timbre Latent Space"** + **"Audio Latent Space Cartography" (arXiv:2212.02610)** `[recent-ish — 2025 / older-foundational]` — the live edge of the idea: a 2D/3D *map* of timbre you move through to synthesize, rather than triggering discrete events. The freshest concrete primary source in this niche is 2025 DAFx; I could **not** find a <30-day primary source for navigable-corpus-cartography specifically (honest note — the field's cutting edge here is months, not weeks, old; logged as such rather than padded).
- **Why it's surprising / the hook:** the lab has a navigable WebGPU particle field (`710-presence-bloom`, its only new ceiling build) and concatenative grains of Karel's real piano (`718-duet-paths`) — but **neither lets you fly through his recording as a place.** Fusing them: every grain of *Welcome Home* becomes one GPU particle positioned by its sonic character (time × brightness), and dragging a cursor through the cloud re-sounds the nearest grains, Hann-windowed and panned. **Could become (and did, this cycle): `720-paths-grainfield`** — descriptor-space cartography of his real piano on a WebGPU compute particle field.
- **This cycle's chain (research → build):** path **(b)/(c)** — extends the ceiling builds 710 (WebGPU compute) + 718 (his-piano concatenative grains) per JURY #3, grounded by CataRT + Audio Latent Space Cartography. Ran a **DEEP** fan-out: ONE concept ("fly inside his recording"), 3 technical approaches — **720-paths-grainfield** (raw WebGPU compute, descriptor map) · **721-paths-constellation** (three.js Points galaxy, attractor steering) · **722-paths-spectral-cloud** (three.js STFT sculpture you orbit/fly — Xenakis UPIC / Ikeda). **Winner: 720** — the only one on WebGPU compute (the lab's scarcest renderer, 1×; JURY #1's literal ask) and the truest cycle-2 of 710. Banked 722 ⭐ (the most novel concept — navigate the *spectral shape*) + 721 as IDEAS §469.

## §470 — 2026-06-18 (UTC) · KIDS dive (cycle 470, WIDE) — **470 % 2 == 0 → kids. JURY 2026-06-18 is the binding verdict, and it is severe: it bans **Canvas2D** (exploded 2×→9× — "the new monoculture renderer"), bans the **shared-clock loop / step-sequencer groove as the core kids idea** ("the new pentatonic / the new harmonic-event"), bans **touch/on-glass-primary input** (5×, two juries running) and the **silly/goofy-comedy kids vibe** (6×). Provocation #1 is explicit: *"Ban Canvas2D for a cycle. 710 proved a WebGPU-compute + MediaPipe stack degrades cleanly — port that **to a kids piece**. A kid deserves the scarce renderer too."* Provocation #5: *"Use Karel's real music — it's in exactly 1 of the last 15."* The question this cycle: is the scarce renderer (WebGPU) actually shippable on the tablets a 4-year-old holds yet, and can a kids piece be built on Karel's REAL piano instead of a fresh oscillator bank?**
**Sources (date-checked this cycle, two WebSearches):**
- **WebGPU is production-ready across ALL browsers as of the 2026 window — including iOS** (`utsubo.com/blog/frontier-web-apis-2026-production-ready`, `programming-helper.com/tech/webgpu-2026…`, `utsubo.com/blog/threejs-2026-what-changed`): Chrome + Firefox + **Safari 26 all ship WebGPU**, covering "the vast majority of web users"; three.js r171+ (Sep 2025) made WebGPU its production path. GPGPU compute (particle sims) runs **10–100× faster than WebGL transform-feedback** and enables compute scenarios WebGL couldn't express. Showcase confirmations of the live register: a *Party* WebGPU particle-physics playground, an audio-reactive WebGPU slime-mold with 19 presets. **Surprising bit:** the historical blocker on giving a *kid* the scarce renderer was always **iOS Safari had no WebGPU** — so every kids WebGPU attempt fell back to Canvas2D on the actual iPad. That blocker **closed in the 2026 window**: WebGPU compute now runs on the iPad a 4-year-old holds, so JURY #1's "a kid deserves the scarce renderer" is, for the first time, *literally shippable* rather than aspirational.
- **The lab's own proven real-piano loader** (grep across `718-duet-paths` / `720-paths-grainfield`): `/api/audio/<id>` GET (recording id `549fc519-…`) → `fetchPianoBuffer` → `buildGrainCorpus` (CataRT-style ~90ms grains, RMS + ZCR-brightness descriptors) → `selectNearest`, with an offline-rendered harmonic fallback so the corpus is never empty. **Surprising bit:** this entire CataRT pipeline — built for *adult* navigable-corpus pieces (718/720) — drops straight into a *kids* toy with one substitution: replace the cursor with a child's **breath**. A 4-year-old humming becomes the descriptor-space navigator of Karel's actual *Welcome Home* performance.
- **No-screen / embodied kids-music register (2026)**: the dominant award-winning kids-audio products (Yoto, Toniebox) are **screen-free / embodied** — squeeze, tap, move — confirming the design pull *away* from finger-on-glass that JURY's touch-input ban also names. Backs a breath/voice-driven input over a touch one for the kids lane.
- **Why it's surprising / the hook:** the lab's only recent *ceiling* build (710-presence-bloom, WebGPU compute) and its only recent use of Karel's real music (718/720) are **both adult**. JURY #1 + #5 asked for exactly their fusion *on the kids side* — and the iOS-WebGPU news makes it real. **Could become (and did): `721-kids-piano-garden`** — a 4-year-old hums and Karel's real *Welcome Home* piano blooms as a WebGPU-compute particle garden (Canvas2D fallback), breath energy → grain count, breath pitch → grain brightness. No beat, no loop, no groove, no silly — a tender luminous texture.
- **This cycle's chain (research → build):** path **(a)** — the iOS-WebGPU-now-shippable finding + the proven CataRT piano loader → a **kids WIDE** fan-out of 3 explorers, each on a *scarce non-banned renderer* and each dodging all four JURY bans (no Canvas2D-primary, no loop-groove, no touch-primary, no silly): **721-kids-piano-garden** (mic-breath → **WebGPU** compute → Karel's-real-piano concatenative grains → tender) · **722-kids-star-scoop** (MediaPipe hands → **three.js** 3D starfield scoop → awe) · **723-kids-aurora-sail** (device-tilt → **WebGL2** GLSL aurora → dreamy). **Winner: 721** — the only candidate hitting *two* jury provocations at once (#1 scarce-renderer-for-a-kid + #5 his real music) and chained to this cycle's research; banked 722 + 723.

## §471 — 2026-06-18 (UTC) · ADULT dive (cycle 471, DEEP) — **the room that re-sounds his own performance.** Binding directives: JURY 2026-06-18 #3 ("stop banking ceilings and walking away — push **710** to a cycle-2, reward DEPTH over breadth; the 2-of-15 ceiling count hasn't moved in two windows because nobody returns to extend"), #5 ("use Karel's REAL music — it's in only 1 of the last 15, all the rest synthesize fresh audio"), #1 (ban Canvas2D — use 710's proven WebGPU-compute stack). The question: what's the named-reference + last-30-days grounding for turning his real recording into **spatialized grains you sculpt with your body in 3D**, fusing the lab's only new ceiling build (710: WebGPU + MediaPipe pose + HRTF) with its concatenative-piano work (718/720)?
**Sources (date-checked this cycle, WebSearch on "concatenative synthesis spatial audio HRTF interactive installation 2026 body tracking corpus grain"):**
- **`Synaptic Sanctum Neuroscape` (2026, ResearchGate publication 400907113)** — a real-time immersive VR installation using a **CataRT-MuBu concatenative granular synthesiser** that produces **spatialised audio grains** whose duration scales with the participant's alert/relaxed state; **IRCAM Spat5 renders each grain with per-source higher-order ambisonics (HOA) binaural decoding locked to head-tracking quaternions.** This is, almost line-for-line, the architecture of this cycle's concept: a body/state signal drives *which* grains of a corpus play and *where they sit in 3D around the listener's head*, binaurally. **Surprising bit:** the freshest installation work (2026) is converging on exactly the fusion the lab has been building in two separate halves — concatenative corpus grains (718/720) + per-source binaural spatialization (710's HRTF voices) — but driven by the *body* as the navigator. Nobody in the lab had yet put the two halves in one piece.
- **Corpus-based concatenative synthesis as a *spatial instrument*** (Schwarz et al., "The Sound Space as Musical Instrument: Playing Corpus-Based Concatenative Synthesis", ResearchGate 265258205; CataRT overview 241312117): grains are selected by **proximity to a target position in descriptor space**, and "by mapping movements in real space to descriptor space, sound segments are triggered continuously, producing a natural and immersive auditory experience." Direct grounding for mapping limb position / body energy → descriptor-space grain selection.
- **The Concatenator (arXiv 2411.04366)** — a Bayesian real-time concatenative *musaicing* approach, confirming the corpus-navigation register is live and being pushed on right now.
- **Why it's surprising / the hook:** the lab BANKS ceilings and walks away (JURY's exact charge) — 710 (the only new ceiling build) sings *synth* D-Dorian voices, and 718/720 (his real piano) live in a *flat 2D* grain map you fly a cursor through. **Could become (and did): a cycle-2 of 710 where the persistent accreting HRTF voices are no longer oscillators but GRAINS OF HIS REAL `Welcome Home` RECORDING** — so the spatial architecture you build with your body around your own ears is made of his actual performance. Three body→grain mapping/temporal models explored.
- **This cycle's chain (research → build):** path **(b/c)** — extend a ceiling build (JURY #3) using this dive's CataRT-spatial grounding → an **adult DEEP** fan-out of 3 approaches to the ONE concept: **722-presence-paths** (descriptor-space *reach* — hand height→brightness, openness→time, dwell-flick places a persistent grain-voice) · **723-presence-tide** (whole-body *conductor* — stillness→sparse/distant/wet grains, vigour→dense/near/dry, no placement) · **724-presence-drift** (each placed voice owns a slow **moving read-head** through his recording, so the spatial chord re-voices itself over minutes — long-form/state). **Winner: 724** — keeps 710's exact placement gesture (a true cycle-2 deepening) AND adds the one axis neither 710 nor 720 had (long-form temporal re-voicing of his real performance); banked 722 + 723.
